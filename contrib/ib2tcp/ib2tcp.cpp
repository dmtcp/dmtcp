...
DmtcpPluginDescriptor_t ib2tcp_plugin = {DMTCP_PLUGIN_API_VERSION,
                                         PACKAGE_VERSION,
                                         "ib2tcp",
                                         "DMTCP",
                                         "dmtcp@ccs.neu.edu",
...
IB2TCP::openListenSocket()
{
  ...
  JNOTE("opened listen socket")
  (listenSock.sockfd())(inet_ntoa(addr_in.sin_addr))(ntohs(addr_in.sin_port));
}
...
IB2TCP::registerNSData()
{
  ...
  for (it = queuePairs.begin(); it != queuePairs.end(); it++) {
    ...
    if (ibqp->localId < ibqp->remoteId) {
      dmtcp_send_key_val_pair_to_coordinator("IB2TCP", &ibqp->localId.lid,
                                             sizeof(ibqp->localId.lid),
                                             &listenAddr, listenAddrLen);
    }
  }
}
...
IB2TCP::sendQueries()
{
  ...
  for (it = queuePairs.begin(); it != queuePairs.end(); it++) {
    ...
    if (ibqp->localId > ibqp->remoteId) {
      ...
      dmtcp_send_query_to_coordinator("IB2TCP", &ibqp->remoteId.lid,
                                      sizeof(ibqp->remoteId.lid),
                                      &ibqp->remoteAddr, &size);
...
IB2TCP::createTCPConnections()
{
  ...
  for (it = queuePairs.begin(); it != queuePairs.end(); it++) {
    ...
    if (ibqp->localId > ibqp->remoteId) {
      ...
      int fd = sock.sockfd();
      JASSERT(fd != -1);
      JASSERT(Util::writeAll(fd, &remoteId, sizeof(remoteId)) ==
              sizeof(remoteId))
      (JASSERT_ERRNO);
      fdToQP[fd] = ibqp;
      qpToFd[ibqp] = fd;
  ...
  for (size_t i = 0; i < numRemaining; i++) {
    ...
    sock.changeFd(fdCounter++);
    JASSERT(Util::readAll(sock.sockfd(), &localId, sizeof(localId)) ==
            sizeof(localId))
    (JASSERT_ERRNO);
...
IB2TCP::modifyQP(struct ibv_qp *qp, struct ibv_qp_attr *attr, int mask)
{
  ...
  if (mask & IBV_QP_PORT) {
    ...
    JASSERT(qp->context->ops.query_port(qp->context, attr->port_num, &qattr) ==
            0);
