...
#define PRINTF(fmt, ...)                                 \
  do {                                                   \
    fprintf(stdout, DEBUG_SIGNATURE fmt, ##__VA_ARGS__); \
  } while (0)
...
start_stop_timer(timer_t timerid, long interval, bool start)
{
  ...
  if (start) {
    ...
    if (timer_gettime(timerid, &its) == 0) {
      JTRACE("Time left: it_value, it_interval")
      (its.it_value.tv_sec)(its.it_value.tv_nsec)(its.it_interval.tv_sec)(
  ...
  if (!start) {
    if (timer_gettime(timerid, &its) == 0) {
      JTRACE("After disabling: it_value, it_interval")
      (its.it_value.tv_sec)(its.it_value.tv_nsec)(its.it_interval.tv_sec)(
        its.it_interval.tv_nsec);
    } else {
      handleError("timer_gettime");
