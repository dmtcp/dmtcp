...
struct internal_ibv_qp
{
  ...
  struct ibv_qp_init_attr
      init_attr; /*!< The attributes used to construct the queue */
  ...
  struct list post_recv_log; /*!< This list contains log entries that track what
recv work
//                                                         requests were posted.
As recv work requests are polled from the CQ,
//                                                         entries in this list
are deleted. */
  struct list post_send_log; /*!< This list contains log entries that track what
send work
//                                                         requests were posted.
As send work requests are polled from the CQ,
//                                                         entries in this list
are deleted. */
  struct list_elem elem;
};
...
{
  struct ibv_qp_attr
      attr; /*!< The attr used in the original call to ibv_modify_qp */
 ...
 */
static inline struct internal_ibv_qp* qp_num_to_qp(struct list* l,
                                                   uint32_t qp_num)
{
  struct list_elem* e;
 ...
 */
static inline struct internal_ibv_dev* ibv_device_to_internal(
 ...
 * */
static inline struct internal_ibv_ctx* ibv_ctx_to_internal(
 ...
 */
static inline struct internal_ibv_comp_channel* ibv_comp_to_internal(
