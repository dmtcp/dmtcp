...
void usage(void)
{
  ...
  fprintf(stderr,
          "-s|-c <serverIP>: run in server mode (-s), or specify server "
          "address (-c <serverIP>) (mandatory)\n");
  fprintf(stderr,
          "-p <port>: port to listen on (if run in server mode) or to connect "
          "to (in client mode), default 55555\n");
...
int main(int argc, char* argv[])
{
  ...
  if (cliserv == CLIENT) {
    ...
    if (setsockopt(
            sock_fd, SOL_SOCKET, SO_REUSEADDR, (char*)&optval, sizeof(optval)) <
        0) {
      perror("setsockopt()");
      exit(1);
    }
