...
static char *request_names[NUM_TUN_REQUEST_TYPES] = {
  "TUNSETNOCSUM", /*
                     ...
                     ver.
                     3.8 */
  "TUNSETDEBUG",     "TUNSETIFF",       "TUNSETPERSIST",
  "TUNSETOWNER",     "TUNSETLINK",      "TUNSETGROUP",
  "TUNSETOFFLOAD",   "TUNSETTXFILTER", /* Can
                                          only
                                          be
                                          set
                                          for
                                          TAP
                                          */
  "TUNSETSNDBUF",    "TUNATTACHFILTER", /* Can
                                           only
                                           be
                                           set
                                           for
...
get_arg(int request, void *arg)
{
  ...
  switch (request) {
    ...
    case TUNSETLINK:
      p_arg = arg;
      break; /* type: int. Setting of link type can only be done
                when the if is down */
    case TUNSETGROUP:
      p_arg = arg;
    ...
    case TUNSETTXFILTER:
      p_arg =
        memcpy(&g_tun_filter, arg, sizeof(struct tun_filter)); /* type:
                                                                  struct
                                                                  tun_filter
                                                                  */
      break;
    ...
    case TUNATTACHFILTER:
      p_arg =
        memcpy(&g_sock_fprog, arg, sizeof(struct sock_fprog)); /* type:
                                                                  struct
                                                                  sock_fprog
                                                                  */
      break;
...
restart()
{
  ...
  if (g_tun_fd != -1) {
    ...
    for (i = 0; i <= g_last_req_idx; i++) {
      ...
      if (ret < 0) {
        snprintf(error_string, MAX_ERROR_STRING_LENGTH, "ERROR: ioctl(%s)\n",
                 request_name);
        perror(error_string);
        if (is_fatal(request)) {
...
DmtcpPluginDescriptor_t tun_plugin = {DMTCP_PLUGIN_API_VERSION,
                                      PACKAGE_VERSION,
                                      "tun",
                                      "DMTCP",
                                      "dmtcp@ccs.neu.edu",
