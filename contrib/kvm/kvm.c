...
static char* g_mp_states[] = {
    "RUNNABLE", "UNINITIALIZED", "INIT_RECEIVED", "HALTED", "SIPI_RECEIVED"};
...
{
  memcpy(&g_kvm_mem_region[mem->slot],
         mem,
         sizeof(struct kvm_userspace_memory_region));
  g_num_of_memory_regions = (mem->slot > g_num_of_memory_regions)
                                ? mem->slot
    ...
    void* addr, size_t length, int prot, int flags, int fd, off_t offset)
{
  ...
  if (fd == g_vcpu_fd) {
    DPRINTF(
        "Saving the vcpu mmap64 params. addr: %p, length: %zu, prot: %d, "
        "flags: %d, offset: %ld\n",
        addr,
        length,
        prot,
...
int ioctl(int fd, unsigned long int request, ...)
{
  ...
  switch ((unsigned int)request) {
    ...
    case KVM_REGISTER_COALESCED_MMIO: {
      memcpy(
          &g_kvm_coalesced_mmio_zone, arg, sizeof(g_kvm_coalesced_mmio_zone));
      break;
    }
    ...
    case KVM_SET_GSI_ROUTING: {
      DPRINTF(
          "Saving GSI routing table. Routing table address: %p, "
          "count: %d\n",
          arg,
          ((struct kvm_irq_routing*)arg)->nr);
      /* NOTE:
  ...
  switch (event) {
    ...
    case DMTCP_EVENT_WRITE_CKPT: {
      ...
      if (dummy == 1) {
        ...
        if (r < 0) {
          DPRINTF(
              "ERROR: Querying IRQCHIP state from the kernel returned: %d\n",
              r);
          DPRINTF("WARNING: Please try checkpointing again\n");
        }
        // dummy = 2;
        ...
        if (data->resumeInfo.isRestart) {
          ...
          if (g_kvm_fd > 0 && g_vm_fd > 0 && g_vcpu_fd > 0) {
            ...
            if (r < 0) {
              ...
              if (NEXT_FNC(mmap)(g_vcpu_mmap_addr,
                                 g_vcpu_mmap_length,
                                 g_vcpu_mmap_prot,
                                 g_vcpu_mmap_flags | MAP_FIXED,
                                 g_vcpu_fd,
                                 0) == MAP_FAILED) {
              ...
              r = NEXT_FNC(ioctl)(
                  g_vcpu_fd, KVM_SET_SIGNAL_MASK, &g_kvm_sigmask);
              if (r < 0) {
              ...
              r = NEXT_FNC(ioctl)(
                  g_vm_fd, KVM_IRQ_LINE_STATUS, &g_kvm_irq_level);
              if (r < 0) {
                DPRINTF("ERROR: Setting IRQ LINE status returned: %d\n", r);
                exit(-1);
              }

              r = NEXT_FNC(ioctl)(g_vm_fd,
              ...
              for (i = 0; i < g_num_of_memory_regions; i++) {
                ...
                DPRINTF("slot:%X, flags:%X, start:%llX, size:%llX, ram:%llX)\n",
                        ...
                        mem->userspace_addr);
                r = NEXT_FNC(ioctl)(
                    g_vm_fd, KVM_SET_USER_MEMORY_REGION, &g_kvm_mem_region[i]);
                if (r < 0) {
                  DPRINTF(
                      "ERROR: Creating memory region #%d returned: \n", i, r);
              ...
              r = NEXT_FNC(ioctl)(
                  g_vm_fd, KVM_SET_GSI_ROUTING, g_kvm_gsi_routing_table);
              if (r < 0) {
                DPRINTF(
                    "ERROR: Setting routing table (#routes=%d) returned: "
                    "%d\n",
                    g_kvm_gsi_routing_table->nr,
                    r);
              }
              ...
              r = NEXT_FNC(ioctl)(
                  g_vcpu_fd, KVM_TPR_ACCESS_REPORTING, &g_kvm_tpr_access_ctl);
              if (r < 0) {
                DPRINTF(
                    "ERROR: Restoring the tpr access reporting returned: %d\n",
                    r);
                DPRINTF("WARNING: Cannot continue\n");
                exit(-1);
              }
              r = NEXT_FNC(ioctl)(
                  g_vcpu_fd, KVM_SET_VAPIC_ADDR, &g_kvm_vapic_addr);
              if (r < 0) {
