...
#define MALLOC_FAMILY_BASIC_SYNC_WRAPPER(ret_type, name, ...)               \
  ...
  do {                                                                      \
    if (SYNC_IS_REPLAY) {                                                   \
      ...
      _real_pthread_mutex_lock(&allocation_lock);                           \
      retval = _almost_real_ ## name(__VA_ARGS__);                                 \
                      ...
                      int fd, off_t offset)
{
  ...
  if (SYNC_IS_REPLAY) {
    ...
    flags |= MAP_FIXED;
    retval = _almost_real_mmap (addr, length, prot | PROT_WRITE, flags, fd, offset);
    ...
    _real_pthread_mutex_lock(&mmap_lock);
    retval = _almost_real_mmap (addr, length, prot, flags, fd, offset);
    if (retval != MAP_FAILED && fd != -1 &&
        ((flags & MAP_PRIVATE) != 0 || (flags & MAP_SHARED) != 0)) {
                         ...
                         int fd, off64_t offset)
{
  ...
  if (SYNC_IS_REPLAY) {
    ...
    flags |= MAP_FIXED;
    retval = _almost_real_mmap64 (addr, length, prot | PROT_WRITE, flags, fd, offset);
    ...
    _real_pthread_mutex_lock(&mmap_lock);
    retval = _almost_real_mmap64 (addr, length, prot, flags, fd, offset);
    if (retval != MAP_FAILED && fd != -1 &&
        ((flags & MAP_PRIVATE) != 0 || (flags & MAP_SHARED) != 0)) {
...
extern "C" int munmap(void *addr, size_t length)
{
  ...
  if (SYNC_IS_REPLAY) {
    ...
    _real_pthread_mutex_lock(&allocation_lock);
    retval = _almost_real_munmap (addr, length);
                        ...
                        size_t new_size, int flags, ...)
{
  ...
  if (SYNC_IS_REPLAY) {
    ...
    retval = _almost_real_mremap (old_address, old_size, new_size, flags, addr);
    JASSERT ( retval == GET_COMMON(currentLogEntry, retval) );
    MALLOC_FAMILY_WRAPPER_REPLAY_END(mremap);
  } else if (SYNC_IS_RECORD) {
    _real_pthread_mutex_lock(&mmap_lock);
    retval = _almost_real_mremap (old_address, old_size, new_size, flags, new_address);
    WRAPPER_LOG_WRITE_ENTRY(my_entry);
    _real_pthread_mutex_unlock(&mmap_lock);
  }
  return retval;
    ...
    size_t new_size, int flags)
{
  ...
  if (SYNC_IS_REPLAY) {
    ...
    _real_pthread_mutex_lock(&mmap_lock);
    retval = _almost_real_mremap (old_address, old_size, new_size, flags, 0);
    WRAPPER_LOG_WRITE_ENTRY(my_entry);
    _real_pthread_mutex_unlock(&mmap_lock);
  }
  return retval;
