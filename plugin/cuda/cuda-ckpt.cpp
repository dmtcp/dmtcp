...
#include "constants.h"
#include "dmtcp.h"
#define DMTCP_LOG_COMPONENT "cuda"
...
int inspect_pipes(pipe_info_t *pipe_fd_array, int max_pipe_fds) {
  ...
  int count = 0;
  ASSERT_ERRNO((dir = opendir("/proc/self/fd")) != NULL,
               "opendir(/proc/self/fd) failed");
  while ((entry = readdir(dir)) != NULL) {
    ...
    if (strncmp(link_target, "pipe:[", 6) == 0) {
      ASSERT(count < max_pipe_fds, "pipe fd table overflow: max={}",
             max_pipe_fds);
      unsigned long inode = strtoul(link_target + 6, NULL, 10);
      int flags = fcntl(fd, F_GETFL);
      ASSERT_ERRNO(flags != -1, "fcntl(F_GETFL) failed for fd {}", fd);
      pipe_rw_t rw_type = (pipe_rw_t)(flags & O_ACCMODE);
      ASSERT(rw_type == PIPE_READ || rw_type == PIPE_WRITE,
             "unexpected pipe access mode: {}", (int)rw_type);
      ...
      ASSERT_ERRNO(fd_flags != -1, "fcntl(F_GETFD) failed for fd {}", fd);
...
int recreate_pipes(pipe_info_t *pipe_fd_array, int num_fds) {
  ...
  ASSERT_NOT_NULL(handle, "failed to open libc");
  int (*real_pipe)(int*);
  *(void**)(&real_pipe) = dlsym(handle, "pipe");
  ASSERT(dlerror() == NULL, "failed to find symbol for pipe");
  ...
  ASSERT_ERRNO(fd_unique != -1, "open(/dev/zero) failed");
  ...
  for (int i = 0; i < num_fds; i++) {
    ...
    ASSERT(map_count < MAX_PIPE_FDS, "pipe map overflow: max={}", MAX_PIPE_FDS);
    if (!found && pipe_map[map_count].created == 0) {
      int new_fds[2];
      ASSERT_ERRNO((*real_pipe)(new_fds) != -1, "pipe() failed");
  ...
  for (int i = 0; i < num_fds; i++) {
    ...
    ASSERT(new_fd_to_dup != -1, "no recreated pipe fd for inode {}", old_inode);
    int ret = dup2(new_fd_to_dup, old_fd);
    ASSERT_ERRNO(ret != -1, "dup2({}, {}) failed", new_fd_to_dup, old_fd);
    ...
    ASSERT_ERRNO(fcntl(old_fd, F_SETFL,
                       pipe_fd_array[i].status_flags & ~O_ACCMODE) != -1,
                 "fcntl(F_SETFL) failed for fd {}", old_fd);
    ASSERT_ERRNO(fcntl(old_fd, F_SETFD, pipe_fd_array[i].fd_flags) != -1,
                 "fcntl(F_SETFD) failed for fd {}", old_fd);
  }
...
static void checkpoint_gpu() {
  ...
  ASSERT(ret == CUDA_SUCCESS, "cuCheckpointProcessLock failed: {}", (int)ret);
  ...
  ASSERT(ret == CUDA_SUCCESS, "cuCheckpointProcessCheckpoint failed: {}",
         (int)ret);
  ret = cuCheckpointProcessGetState(pid, &cuda_state);
  ASSERT(ret == CUDA_SUCCESS, "cuCheckpointProcessGetState failed: {}",
         (int)ret);
  ASSERT(cuda_state == CU_PROCESS_STATE_CHECKPOINTED,
         "unexpected CUDA state after checkpoint: {}", (int)cuda_state);
  ...
  for (Area device_area : nvidia_device_files) {
    ...
    ASSERT(ret == device_area.addr,
           "mmap reserve mismatch: got={} want={} name={}",
           ret, device_area.addr, device_area.name);
  }
}
...
static void restore_gpu() {
  ...
  ASSERT(ret == CUDA_SUCCESS, "cuCheckpointProcessRestore failed: {}", (int)ret);
  ret = cuCheckpointProcessUnlock(pid, &unlock_args);
  ASSERT(ret == CUDA_SUCCESS, "cuCheckpointProcessUnlock failed: {}", (int)ret);
  ret = cuCheckpointProcessGetState(pid, &cuda_state);
  ASSERT(ret == CUDA_SUCCESS, "cuCheckpointProcessGetState failed: {}",
         (int)ret);
  ASSERT(cuda_state == CU_PROCESS_STATE_RUNNING,
         "unexpected CUDA state after restore: {}", (int)cuda_state);
...
static void cuda_event_hook(DmtcpEvent_t event, DmtcpEventData_t *data) {
  switch (event) {
    case DMTCP_EVENT_INIT:
      ...
      // the "DMTCP:" program name prefix must NOT be applied. To disable the
      // program name prefix, we need to set the environment variable.
      ASSERT(setenv(ENV_VAR_DISABLE_PRGNAME_PREFIX, "1", 1) == 0,
             "Can't disable DMTCP program name prefix.");
    ...
    case DMTCP_EVENT_PRECHECKPOINT:
      ...
      ASSERT(num_fds_found >= 0, "inspect_pipes failed: {}", num_fds_found);
      break;
