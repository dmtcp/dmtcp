...
EXTERNC int dmtcp_modify_env_enabled()
{
  return 1;
}
...
{
  if (getenv(envName)) strncpy(dest, getenv(envName), size);
...
char* read_dmtcp_env_file(char* file, int size)
{
  ...
  if (strlen(dmtcp_get_ckpt_dir()) > sizeof(pathname) - 1 - sizeof(file)) {
    fprintf(stderr,
            "%s:%d : Pathname of ckpt dir is too long: %s\n",
            __FILE__,
            __LINE__,
            dmtcp_get_ckpt_dir());
    exit(1);
        ...
        // Env name after '$' may consist only of alphanumeric char's and '_'
        {
          ...
          *d = '\0';
          ...
          if (isNameChanged && getenv(envName)) {
            ...
            rc = dmtcp_get_restart_env(
                envName, dest, sizeof(valueBuf) - (dest - valueBuf));
          }
          if (rc == 0)
