...
static void ptrace_attach_threads(int isRestart)
{
  ...
  for (size_t i = 0; i < inferiors.size(); i++) {
    ...
    if (!inf->isCkptThread()) {
      ...
      JASSERT(_real_ptrace(
                  PTRACE_SETOPTIONS, inferior, 0, inf->getPtraceOptions()) !=
              -1)
...
static void ptrace_single_step_thread(Inferior* inferiorInfo, int isRestart)
{
#if defined(__i386__) || defined(__x86_64__)
  struct user_regs_struct regs;
#elif defined(__arm__)
...
#warning "TODO: Implementation for ARM64."
    peekdata =
        _real_ptrace(PTRACE_PEEKDATA, inferior, (void*)aarch64_regs.pc, 0);
    long inst = peekdata & 0xffff;
...
#endif
    {
      if (isRestart) { /* Restart time. */
        ...
        if (last_command == PTRACE_SINGLESTEP) {
          ...
          addr += EFLAGS_OFFSET;
          errno = 0;
          JASSERT((int)(eflags = _real_ptrace(
                            PTRACE_PEEKDATA, inferior, (void*)addr, 0)) != -1)
          (superior)(inferior)(JASSERT_ERRNO);
          eflags |= 0x0100;
          JASSERT(_real_ptrace(
                      PTRACE_POKEDATA, inferior, (void*)addr, (void*)eflags) !=
                  -1)
          (superior)(inferior)(JASSERT_ERRNO);
...
extern "C" long ptrace(enum __ptrace_request request, ...)
{
  ...
  if (ptrace_ret != -1) {
    PtraceInfo::instance().processSuccessfulPtraceCmd(request, pid, addr, data);
  }
