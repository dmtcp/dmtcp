...
static void setup_signals_ini()
{
  if (signal(SIGHUP, SIG_IGN) != SIG_IGN) signal(SIGHUP, signal_handler_ini);
  if (signal(SIGINT, SIG_IGN) != SIG_IGN) signal(SIGINT, signal_handler_ini);
  if (signal(SIGQUIT, SIG_IGN) != SIG_IGN) signal(SIGQUIT, signal_handler_ini);
  if (signal(SIGTERM, SIG_IGN) != SIG_IGN) signal(SIGTERM, signal_handler_ini);
...
static void signal_handler_rstr(int sig)
{
  quit_pending = 1;
  if (sig == SIGCHLD) {
    // TODO: wait stuff here?
...
static void setup_signals_rstr()
{
  if (signal(SIGHUP, SIG_IGN) != SIG_IGN) signal(SIGHUP, signal_handler_rstr);
  if (signal(SIGINT, SIG_IGN) != SIG_IGN) signal(SIGINT, signal_handler_rstr);
  if (signal(SIGQUIT, SIG_IGN) != SIG_IGN) signal(SIGQUIT, signal_handler_rstr);
  if (signal(SIGTERM, SIG_IGN) != SIG_IGN) signal(SIGTERM, signal_handler_rstr);
...
static void prepare_initial_helper(char* sync_path)
{
  char* path;
...
static void create_stdio_fds_rstr(int* in, int* out, int* err)
{
  struct stat buf;
...
int main(int argc, char** argv, char** envp)
{
  ...
  if (!restart_helper) {
    ...
    slurm_srun_handler_register(
        srun_stdin, srun_stdout, srun_stderr, &srun_pid);
    if (is_empty_loop()) {
      while (!quit_pending) {
        sleep(1);
      }
