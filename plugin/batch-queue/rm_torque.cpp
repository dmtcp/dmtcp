...
typedef int (*tm_spawn_t)(int argc,
                          char** argv,
                          char** envp,
                          tm_node_id where,
                          tm_task_id* tid,
...
bool isTorqueFile(string relpath, string& path)
{
  ...
  switch (_get_rmgr_type()) {
    case Empty:
      probeTorque();
      if (_get_rmgr_type() != torque) return false;
      break;
    case torque:
      break;
    default:
  ...
  if (torque_home().size() == 0) return false;

  string abspath = torque_home() + "/" + relpath;
  JTRACE("Compare path with")(path)(abspath);
  if (path.size() < abspath.size()) return false;

  if (path.substr(0, abspath.size()) == abspath) return true;
...
bool isTorqueIOFile(string& path)
{
  ...
  if (isTorqueFile("spool", path)) return true;
...
{
  if (!isTorqueIOFile(path)) return false;
...
extern "C" int tm_spawn(int argc,
                        char** argv,
                        char** envp,
                        tm_node_id where,
                        tm_task_id* tid,
                        tm_event_t* event)
{
  ...
  if (libtorque_init()) return TM_BADINIT;

  char dmtcpCkptPath[PATH_MAX] = "";
  string ckptCmdPath = Util::getPath("dmtcp_launch");
  ret = Util::expandPathname(
      ckptCmdPath.c_str(), dmtcpCkptPath, sizeof(dmtcpCkptPath));
  ...
  const char* new_argv[argc + (dsize + 1)]; // (dsize+1) is DMTCP part including
  // dmtcpCkptPath
  string cmdline;
  size_t i;
  ...
  JTRACE("CMD:")(cmdline);
  ret =
      tm_spawn_ptr(argc + dsize + 1, (char**)new_argv, envp, where, tid, event);
                      ...
                      int type)
{
  ...
  if (type == TORQUE_NODE) {
    ...
    tempfd = _real_open(newpath.c_str(),
                        O_CREAT | O_WRONLY,
