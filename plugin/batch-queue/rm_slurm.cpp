...
void dmtcp::slurm_restore_env()
{
  ...
  if (!fp) {
    JTRACE(
        "Cannot open SLURM environment file. Environment won't be restored!")(
        filename);
    return;
  }
 ...
 * new :
 * dmtcp_srun_helper dmtcp_nocheckpoint | srun <srun-opts> | dmtcp_launch
 *<dmtcp-opts> --explicit-srun |  <binary> <binary-opts>
 * <---------- dmtcp hijack -----------> <-orig srun------> <------------- dmtcp
 *hijack --------------> <----- orig binary----->
 *
 * This complexy is because we want to launch dmtcp srun helper that will
 *knowingly be under
...
static int patch_srun_cmdline(char* const argv_old[], char*** _argv_new)
{
  ...
  *_argv_new = (char**)JALLOC_HELPER_MALLOC(vect_size);
  ...
  // Move srun part like: srun --nodes=3 --ntasks=3 --kill-on-bad-exit
  // --nodelist=c2909,c2911,c2913
  ...
  for (; old_pos < argc_old; old_pos++) {
    if (argv_old[old_pos][0] == '-') {
      ...
      if (argv_old[old_pos][1] != '-' && (strlen(argv_old[old_pos]) == 2)) {
        ...
        // In practice some MPI implementation (i.e. Intel MPI) ignore this rule
        // and use the following syntax:
        // "--nodelist node1,node2,..." this causes full option to be splitted
        // onto two argv strings.
        // Here we handle ony those options that is the point of interest for
        // MPI librarys.
  ...
  // Copy dmtcp part so final command looks like: srun <opts> dmtcp_launch
  // <dmtcp_options> orted <orted_options>
                      ...
                      char* const envp[])
{
  ...
  JASSERT(dmtcp::Util::expandPathname(
              srunHelper, helper_path, sizeof(helper_path)) == 0);
  // We need this step to protect stdio fd's from being opened for other
  // purposes.
  occupate_stdio();
...
int dmtcp::slurmRestoreFile(const char* path,
