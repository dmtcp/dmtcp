...
{
  string serv_names[] = {// Open MPI
                         "orted",
                         // MPICH/Hydra
                         "hydra_pmi_proxy"
                         // To be continued ...
  };
  pmtype_t ptmap[] = {// Open MPI
                      pm_orte,
                      // Hydra
                      pm_hydra};
...
{
  string serv_names[] = {// DMTCP
                         "dmtcp_srun_helper"};
  uint size = sizeof(serv_names) / sizeof(serv_names[0]);
  uint i;
  for (i = 0; i < size; i++) {
    if (str.find("ckpt_" + serv_names[i]) != string::npos) return true;
  }
  return false;
}

void resources_input::split2slots(std::string& str,
...
bool resources_input::add_host(string& str, uint& node_id)
{
  ...
  // get host name
  if ((match_pos = str.find(delim)) == string::npos) return false;
  ...
  // skip mode
  if ((match_pos = str.find(delim, start_pos)) == string::npos) return false;
  if (!(match_pos - start_pos > 0))
    return false;
  else {
    size_t sublen = match_pos - start_pos;
...
resources_input::resources_input(string str) : resources(input)
{
  ...
  if ((match_pos = str.find(delim)) == string::npos) return;
  start_pos = match_pos + delim.length();
  while ((match_pos = str.find(delim, start_pos)) != string::npos) {
    size_t sublen = match_pos - start_pos;
    if (sublen > 0) {
      string sub(str.substr(start_pos, sublen));
      if (add_host(sub, hostid)) _valid = true;
    }
    start_pos = match_pos + delim.length();
  }
...
void resources_input::writeout_old(string env_var, resources& r)
{
  ...
  if (!map_to(r, map, warning)) {
    cout << "DMTCP_DISCOVER_RM_ERROR=\'Cannot map initial resources into the "
            "restart allocation\'" << endl;
    return;
  }
  if (warning != "") {
