...
template <typename T>
static bool
__sync_bool_compare_and_swap(T volatile *ptr, T oldval, T newval)
{
  bool retval = false;
...
public:
  ...
  void initialize(int blockSize) { _blockSize = blockSize; }
  ...
  int numExpands() { return _numExpands; }
  void preExpand()
  {
...
protected:
  ...
  {
    _numExpands++;
    if (_root != NULL && fred_record_replay_enabled &&
        fred_record_replay_enabled()) {
...
operator new(size_t nbytes)
{
  ...
  size_t *p =
    (size_t *)jalib::JAllocDispatcher::allocate(nbytes + sizeof(size_t));
  *p = nbytes;
  p += 1;
  return p;
}
