...
#define JSERIALIZE_ASSERT_POINT(str)                                        \
  {                                                                         \
    ...
    o &versionCheck;                                                        \
    JASSERT(versionCheck == correctValue)                                   \
    (versionCheck)(correctValue)(o.filename()).Text("invalid file format"); \
  }
...
public:
  ...
  JBinarySerializer(const jalib::string &filename)
    : _filename(filename), _bytes(0)
  {
  }
  ...
  void serializeMap(dmtcp::map<K, V> &t)
  {
    ...
    if (isReader()) {
      ...
      for (typename dmtcp::map<K, V>::iterator i = t.begin(); i != t.end();
           ++i) {
        K key = i->first;
        V val = i->second;
        serializePair(key, val);
