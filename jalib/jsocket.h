...
public:
  ...
  bool connect(const struct sockaddr *addr, socklen_t addrlen, int port = -1);
  ...
  template <typename T>
  JSocket &operator>>(T &t)
  {
    readAll((char *)&t, sizeof(T));
    return *this;
  }
  ...
  int port() const
  {
    ...
    if (-1 == getsockname(_sockfd, (struct sockaddr *)&addr, &addrlen)) {
      return -1;
    } else {
...
{
public:
  JServerSocket(int sockfd) : JSocket(sockfd) { enablePortReuse(); }
