 ...
 *
 * code:
 *     JNOTE("Values of abcd (in the form 'a=1') will be printed below this
 * text.")(a)(b)(c)(d);
 * outputs:
 *     JNOTE at main.cpp:15 in main
 *     Reason: Values of abcd (in the form 'a=1') will be printed below this
 * text.
 *       a = 1
 *       b = 2
 *       c = 3
...
#define JASSERT_CONTEXT(type, reason) \
  Print('[')                          \
    ...
    .Print("] " type " at ")          \
    .Print(JASSERT_FILE)              \
    .Print(":" JASSERT_LINE " in ")   \
    .Print(JASSERT_FUNC)              \
    .Print("; REASON='" reason "'\n")

#ifdef DEBUG
#define JTRACE(msg) \
  jassert_internal::JAssert(false).JASSERT_CONTEXT("TRACE", msg).JASSERT_CONT_A
#else // ifdef DEBUG
#define JTRACE(msg) \
  if (true) {       \
  } else            \
...
#define JWARNING(term)                                        \
  ...
  jassert_internal::JAssert(false)                            \
    .JASSERT_CONTEXT("WARNING", "JWARNING(" #term ") failed") \
    .JASSERT_CONT_A
