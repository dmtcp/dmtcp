...
required_env(const char *name)
{
  ...
  if (value == NULL || value[0] == '\0') {
    fprintf(stderr, "tmpdir: %s is not set\n", name);
    exit(1);
  }
  return value;
}
...
main(void)
{
  const char *tmpdir_root = required_env("DMTCP_TMPDIR_TEST_ROOT");
  const char *success_file = required_env("DMTCP_TMPDIR_SUCCESS_FILE");
  int wrote_success = 0;
  ...
  while (1) {
    ...
    if (!wrote_success) {
      ...
      if (tmpdir == NULL || !path_is_under(tmpdir, tmpdir_root)) {
        fprintf(stderr,
                "tmpdir: tmpdir %s is not under requested root %s\n",
