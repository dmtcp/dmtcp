...
static const char file_names_1[NUM_OF_FILES][FILENAME_LEN] = {
  "bake_0", "bake_1", "bake_2"};
static const char file_names_2[NUM_OF_FILES][FILENAME_LEN] = {
  "Samm_3", "Samm_4", "Samm_5"};
...
main(int argc, char **argv)
{
  ...
  if ((ret_val = mkdir(directory_1, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH)) !=
      SUCCESS) {
    printf("couldn't create the directory...it probably already exists\n");
  }

  if ((ret_val = mkdir(directory_2, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH)) !=
      SUCCESS) {
    printf("couldn't create the directory...it probably already exists\n");
  }
   ...
   *========================================*/
  wd_1 = inotify_add_watch(fd, directory_1, IN_OPEN | IN_CLOSE | IN_MODIFY);
  if (wd_1 < 0) {
   ...
   * and it's monitor thread
   *========================================*/
  wd_2 = inotify_add_watch(fd, directory_2, IN_OPEN | IN_CLOSE | IN_MODIFY);
  if (wd_2 < 0) {
    perror("inotify_add_watch");
    ret_val = FAILURE;
    goto INOTIFY_SIMPLE_TEST_LBL_EXIT;
  ...
  for (i = 0; i < NUM_OF_FILES; i++) {
    ...
    if (sizeof(name_path_1) < strlen(directory_1) + sizeof(file_names_1[i]) ||
        sizeof(name_path_2) < strlen(directory_2) + sizeof(file_names_2[i])) {
  ...
  while (1) {
    ...
    while (event_index < len) {
      ...
      if (event->mask & IN_OPEN) {
        ...
        printf("unknown event occurred:%d, event name:%s", event->mask,
               event->name);
      }
