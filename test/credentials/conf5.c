...
void
parse_credentials(
  int ret, char *name, pid_t pid, pid_t ppid, pid_t pgrp, pid_t sid)
{
  char buf[256];
  int len = 0;
  int cnt = 0;

  len += sprintf(buf, "%s: mismatch in (err=%02x,eq_ppid=%d,eq_sid=%d): ", name,
                 ret, eq_ppid, eq_sid);
  if ((ret & PID_MISM)) {
    cnt++;
...
main()
{
  ...
  if (!(p1_cids[2] = fork())) {
    ...
    if (!(p13_cids[0] = fork())) {
      eq_ppid = 1; // after restart we should find thi same
      // PPID
      process("p131", NULL, 0);
    }
    ...
    if (!(ret = fork())) {
      // process which will be terminated
      if (!fork()) {
        // p2
        pid_t p2_cids[2];
        sleep(1);
