...
class SyntheticCoordinatorWorkerTest(unittest.TestCase):
    def test_positive_integer_options_are_not_limited_to_port_range(self):
    ...
    def coordinator_status(self, port):
        ...
        return DmtcpCommandJson.parse(result.stdout)

    def assertCommandSuccess(self, payload, command=None):
        self.assertEqual(payload.get("command_status"),
                         DmtcpCommandJson.SUCCESS_STATUS)
        self.assertNotIn("ok", payload)
        self.assertNotIn("type", payload)
        if command is not None:
            self.assertEqual(payload.get("command"), command)

    def assertCommandFailure(self, payload, status, command=None):
        self.assertEqual(payload.get("command_status"), status)
        self.assertNotIn("ok", payload)
        self.assertNotIn("type", payload)
        if command is not None:
            self.assertEqual(payload.get("command"), command)
    ...
    def test_checkpoint_command_reaches_synthetic_worker(self):
        with CoordinatorFixture() as coordinator:
            ...
            try:
                ...
                self.assertCommandSuccess(payload)
    ...
    def test_checkpoint_command_rejects_second_request_while_active(self):
        with CoordinatorFixture() as coordinator:
            ...
            try:
                ...
                self.assertCommandFailure(payload, "DMT_COORD_NOT_RUNNING")
                self.assertEqual(payload["command"], "DMT_CHECKPOINT")
