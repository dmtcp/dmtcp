...
| Sanitizers | `tsan`, `tsan-clang`, `tsan-clang-static` |
...
| `tsan`, `tsan-clang`, `tsan-clang-static` | One cycle (`cycles=1`) | These validate that a ThreadSanitizer-instrumented target (gcc `libtsan`, clang `-shared-libsan`, and clang's static runtime) launches, runs, and survives a single checkpoint/restart under DMTCP. A second consecutive restart is a known limitation (the checkpoint thread mutates its own TSAN trace state while that memory is being written, so the image can capture an inconsistent trace-part list that faults on the next restart); single checkpoint/restart is unaffected. |
