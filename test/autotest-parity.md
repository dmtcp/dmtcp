...
| File, fd, and path behavior | `file1`, `file2`, `file3`, `stat`, `mmap1`, `mremap`, `zeropages`, `zeropages-pread`, `guardpage`, `guardpage-pread`, `poll`, `shared-fd1`, `shared-fd2`, `stale-fd`, `procfd1`, `epoll1`, `epoll2`, `gzip` |
...
| `plugin-init-env` | Ported | This validates the internal `DMTCP_PLUGIN` equivalent of launcher `--with-plugin` that is used in an external plugin. |
