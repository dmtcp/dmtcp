...
static const char* theRestartScriptHeader =
    ...
    "#     prompts.\n"
    "#  3. Verify that the dmtcp_restart command is in your path on all "
    "hosts,\n"
...
static const char* theRestartScriptCheckLocal =
    ...
    "  if [ -z \"$worker_ip\" ]; then\n"
    "    worker_ip=$(nslookup $worker_host | grep -A1 'Name:' | grep "
    "'Address:' | sed -e 's/Address://' -e 's/ //' -e 's/	//')\n"
    "  fi\n"
    "  if [ -z \"$worker_ip\" ]; then\n"
    "    worker_ip=$(getent ahosts $worker_host |grep "
    "\"^[0-9]\\+\\.[0-9]\\+\\.[0-9]\\+\\.[0-9]\\+ *STREAM\" | cut -d' ' -f1)\n"
    "  fi\n"
    "  if [ -z \"$worker_ip\" ]; then\n"
    ...
    "  fi\n"
    "  output=$($ifconfig_path -a | grep \"inet addr:.*${worker_ip} "
    ".*Bcast\")\n"
    "  if [ -n \"$output\" ]; then\n"
    "    is_local_node=1\n"
    "  else\n"
...
static const char* slurmHelperContactFunction =
    ...
    "    echo \"DMTCP_SRUN_HELPER_ADDR=$DMTCP_SRUN_HELPER_ADDR\" >> "
    "$SLURM_ENV_FILE\n"
    "  done\n"
    "}\n\n\n";
...
static const char* theRestartScriptUsage =
    ...
    "        checkpoint image to be restarted by a different user than the "
    "one\n"
    "        that create it. (environment variable "
    "DMTCP_DISABLE_UID_CHECKING)\n"
...
static const char* theRestartScriptCmdlineArgHandler =
    ...
    "          shift; shift;;\n"
    "        *)\n"
    "          echo \"$0: unrecognized option \'$1\'. See correct usage "
    "below\"\n"
    "          echo \"$usage_str\"\n"
    "          exit;;\n"
    "      esac\n"
...
static const char* theRestartScriptMultiHostProcessing =
    ...
    "restart_modes=$(\\\n"
    "  echo $worker_ckpts | sed -e \'s/\'\"$worker_ckpts_regexp\"\'/: "
    "\\2/g\')\n"
    "ckpt_files_groups=$(\\\n"
    "  echo $worker_ckpts | sed -e \'s/\'\"$worker_ckpts_regexp\"\'/: "
    "\\3/g\')\n"
    ...
    "\\t\\n]*\\([^:]*\\).*/\\1/\')\n"
    "  ckpt_files_groups=$(echo $ckpt_files_groups | sed -e "
    "\'s/[^:]*:[^:]*//\')\n"
    "\n"
    ...
    "  if [ \"$is_local_node\" -eq 1 -o \"$num_worker_hosts\" == \"1\" ]; "
    "then\n"
    ...
    "      $remote_dmt_rstr_cmd --host \"$coord_host\" --port "
    "\"$coord_port\"\\\n"
    ...
    "      \"/bin/sh -c \'$remote_dmt_rstr_cmd --host $coord_host --port "
    "$coord_port\\\n"
    "      $ckpt_dir --join --interval \"$checkpoint_interval\" $tmpdir \\\n"
    "      $new_ckpt_files_group\'\" &\n"
    "  fi\n\n"
    ...
    "  $ckpt_dir $maybejoin --interval \"$checkpoint_interval\" $tmpdir "
    "$noStrictUidChecking $localhost_ckpt_files_group\n"
    "fi\n\n"
...
static uint32_t theDefaultCheckpointInterval =
    0; /* Reset to this on new comp. */
                                         ...
                                         DmtcpMessage* reply /*= NULL*/)
{
  ...
  switch (cmd) {
    ...
    case 'B': // prefix blocking command, prior to checkpoint command
      ...
      break;
    case 'x':
    case 'X': // prefix exit command, prior to checkpoint command
    ...
    case 'K':
      ...
      // FIXME: What happens if a 'k' command is followed by a 'c' command
      // before
...
void DmtcpCoordinator::updateMinimumState(WorkerState oldState)
{
  ...
  if (oldState == WorkerState::RUNNING && newState == WorkerState::SUSPENDED) {
...
void DmtcpCoordinator::onData(CoordClient* client)
{
  ...
  switch (msg.type) {
    ...
    case DMT_REGISTER_NAME_SERVICE_DATA_SYNC: {
      JTRACE("received REGISTER_NAME_SERVICE_DATA_SYNC msg")(
          client->identity());
      lookupService.registerData(msg, (const void*)extraData);
    ...
    case DMT_NAME_SERVICE_QUERY: {
      ...
      lookupService.respondToQuery(client->sock(), msg, (const void*)extraData);
    } break;
#endif
...
{
  ostringstream o;
  o << Util::getTmpDir() << "/dmtcpSharedArea." << compId << "." << std::hex
    << curTimeStamp;
...
void DmtcpCoordinator::onConnect()
{
  ...
  if (hello_remote.type == DMT_NAME_SERVICE_WORKER) {
    CoordClient* client =
        new CoordClient(remote, &remoteAddr, remoteLen, hello_remote);
  ...
  if (killInProgress) {
    JNOTE(
        "Connection request received in the middle of killing computation. "
  ...
  if (hello_remote.type == DMT_RESTART_WORKER) {
    if (!validateRestartingWorkerProcess(
            hello_remote, remote, &remoteAddr, remoteLen)) {
      return;
    }
    ...
    if (!validateNewWorkerProcess(
            hello_remote, remote, client, &remoteAddr, remoteLen)) {
      return;
    }
    _virtualPidToClientMap[client->virtualPid()] = client;
  } else {
    ...
    socklen_t remoteLen)
{
  ...
  if (workersRunningAndSuspendMsgSent == true) {
    ...
    JNOTE(
        "Current computation not in RUNNING state."
    ...
    if (compId == UniquePid(0, 0, 0)) {
      ...
      if (!client->prefixDir().empty() && client->hostname() != localHostName) {
        if (remotePrefix.empty()) {
          ...
          JNOTE(
              "This node has different prefixDir than the rest of the "
...
DmtcpCoordinator::ComputationStatus DmtcpCoordinator::getStatus() const
{
  ...
  if (status.minimumState == WorkerState::CHECKPOINTED && isRestarting &&
      count < numPeers) {
    JTRACE(
        "minimal state counted as CHECKPOINTED but not all processes"
...
void DmtcpCoordinator::writeRestartScript()
{
  ...
  o << string(ckptDir) << "/" << RESTART_SCRIPT_BASENAME << "_" << compId;
  ...
  fprintf(fp,
          ...
          "\n"
          "if test -z \"$" ENV_VAR_NAME_HOST
          "\"; then\n"
          "  coord_host=%s\nfi\n\n"
          "coord_port=$" ENV_VAR_NAME_PORT
          "\n"
          "if test -z \"$" ENV_VAR_NAME_PORT
          "\"; then\n"
          "  coord_port=%d\nfi\n\n"
          "checkpoint_interval=$" ENV_VAR_CKPT_INTR
          "\n"
          "if test -z \"$" ENV_VAR_CKPT_INTR
          "\"; then\n"
  ...
  fprintf(fp,
          "# Number of hosts in the computation = %zd\n"
  ...
  if (isSingleHost) {
    ...
    for (host = _restartFilenames.begin(); host != _restartFilenames.end();
         ++host) {
    ...
    fprintf(
        ...
        "      rm $DMTCP_SRUN_HELPER_SYNCFILE\n"
        "      dmtcp_restart --join --host $DMTCP_HOST --port $DMTCP_PORT "
        "$DMTCP_LAUNCH_CKPTS\n"
        "    fi\n"
        "    exit 0\n"
        ...
        "        exit 0\n"
        "    fi\n"
        "    arguments=\"PATH=$PATH DMTCP_HOST=$DMTCP_HOST "
        ...
        "      eval \"val=\\${DMTCP_REMLAUNCH_${i}_SLOTS}\"\n"
        "      arguments=$arguments\" "
        "DMTCP_REMLAUNCH_${i}_SLOTS=\\\"$val\\\"\"\n"
        "      bound2=$(($val - 1))\n"
        "      for j in $(seq 0 $bound2); do\n"
        "        eval \"ckpts=\\${DMTCP_REMLAUNCH_${i}_${j}}\"\n"
        "        arguments=$arguments\" "
        "DMTCP_REMLAUNCH_${i}_${j}=\\\"$ckpts\\\"\"\n"
        "      done\n"
        "    done\n"
...
static void calcLocalAddr()
{
  ...
  if (error == 0) {
    ...
    for (res = result; res != NULL; res = res->ai_next) {
      ...
      error = getnameinfo(
          res->ai_addr, res->ai_addrlen, name, NI_MAXHOST, NULL, 0, 0);
      if (error != 0) {
...
int main(int argc, char** argv)
{
  ...
  if (fcntl(PROTECTED_COORD_FD, F_GETFD) != -1) {
    ...
    JASSERT(listenSock->isValid())(thePort)(JASSERT_ERRNO).Text(
