...
static const char *theUsage =
  ...
  "  --exit-after-ckpt\n"
  "      Kill peer processes of computation after first checkpoint is "
  "created\n"
  "  --daemon\n"
  ...
  "  -q, --quiet \n"
  "      Skip startup msg; Skip NOTE msgs; if given twice, also skip "
  "WARNINGs\n"
  "  --help:\n"
...
DmtcpCoordinator::printStatus(size_t numPeers, bool isRunning)
{
  ...
  o << "Status..." << std::endl
    << "Host: " << coordHostname << " (" << inet_ntoa(localhostIPAddr) << ")"
    << std::endl
...
DmtcpCoordinator::recordCkptFilename(CoordClient *client, const char *extraData)
{
  ...
  if (_numRestartFilenames == _numCkptWorkers) {
    const string restartScriptPath = RestartScript::writeScript(
      ckptDir, uniqueCkptFilenames, ckptTimeStamp, theCheckpointInterval,
      thePort, compId, _restartFilenames);
...
DmtcpCoordinator::onData(CoordClient *client)
{
  ...
  switch (msg.type) {
    ...
    case DMT_NULL:
      JWARNING(false)
      (msg.type).Text("unexpected message from worker. Closing connection");
      onDisconnect(client);
      break;
    default:
...
removeStaleSharedAreaFile()
{
  ...
  o << tmpDir << "/dmtcpSharedArea." << compId << "." << std::hex
    << curTimeStamp;
...
DmtcpCoordinator::onConnect()
{
  ...
  if (hello_remote.type == DMT_RESTART_WORKER) {
    if (!validateRestartingWorkerProcess(hello_remote, remote, &remoteAddr,
                                         remoteLen)) {
      return;
    }
    ...
    if (!validateNewWorkerProcess(hello_remote, remote, client, &remoteAddr,
                                  remoteLen)) {
      return;
    }
    _virtualPidToClientMap[client->virtualPid()] = client;
  ...
  socklen_t remoteLen)
{
  ...
  if (compId == UniquePid(0, 0, 0)) {
    ...
    (minimumState()).Text("Coordinator should be idle at this moment");
...
DmtcpCoordinator::startCheckpoint()
{
  ...
  if (s.minimumState == WorkerState::RUNNING && s.minimumStateUnanimous &&
    ...
    if (s.numPeers > 0) {
      JTRACE("delaying checkpoint, workers not ready")
