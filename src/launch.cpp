...
void
Launch::initializeLaunch(int *argc, const char ***argv)
{
  ResetProtectedFds();
...
void
Launch::validateLaunchEnvironment(int argc, const char **argv)
{
...
const char **
Launch::patchArgvForSetuid(int argc, const char **argv)
{
  ...
  const char **screenArgv = NULL;
  if (testScreen(argv, &screenArgv)) {
    return screenArgv;
  }
...
void
Launch::setLDPreloadLibs(int argc, const char **argv)
{
...
DmtcpInfo dmtcp_register_new_process(int *argc, const char ***argv)
{
  ...
  info.argv = Launch::patchArgvForSetuid(*argc, *argv);
  info.argc = *argc;

  Launch::setLDPreloadLibs(*argc, info.argv);
