...
void _dmtcp_lock()
{
  _real_pthread_mutex_lock(&theMutex);
}
void _dmtcp_unlock()
{
  _real_pthread_mutex_unlock(&theMutex);
}
...
void initialize_libc_wrappers()
{
  ...
  if (!_libc_wrappers_initialized) {
     ...
     */
    _real_func_addr[ENUM(pthread_create)] =
        dlvsym(RTLD_NEXT, "pthread_create", "GLIBC_2.1");
#endif
...
#define REAL_FUNC_PASSTHROUGH_WORK(name)                               \
  if (fn == NULL) {                                                    \
    ...
    if (fn == NULL) {                                                  \
      fprintf(stderr,                                                  \
              "*** DMTCP: Error: lookup failed for %s.\n"              \
...
{
  REAL_FUNC_PASSTHROUGH_TYPED(int, pthread_cond_timedwait)(
      cond, mutex, abstime);
}
...
SYSCALL_ARG_RET_TYPE _real_syscall(SYSCALL_ARG_RET_TYPE sys_num, ...)
{
  ...
  va_start(ap, sys_num);
  for (i = 0; i < 7; i++) arg[i] = va_arg(ap, void*);
  va_end(ap);
  ...
  REAL_FUNC_PASSTHROUGH_TYPED(SYSCALL_ARG_RET_TYPE, syscall)(
      sys_num, arg[0], arg[1], arg[2], arg[3], arg[4], arg[5], arg[6]);
}
...
LIB_PRIVATE
int _real_clone(int (*function)(void*),
                void* child_stack,
                int flags,
                void* arg,
                int* parent_tidptr,
                ...
                int* child_tidptr)
{
  REAL_FUNC_PASSTHROUGH(__clone)(
      function, child_stack, flags, arg, parent_tidptr, newtls, child_tidptr);
}
...
int _real_pthread_timedjoin_np(pthread_t thread,
                               ...
                               const struct timespec* abstime)
{
  REAL_FUNC_PASSTHROUGH_TYPED(int, pthread_timedjoin_np)(
      thread, retval, abstime);
}
...
{
  REAL_FUNC_PASSTHROUGH_TYPED(ssize_t, mq_timedreceive)(
...
int _real_mq_timedsend(mqd_t mqdes,
                       ...
                       const struct timespec* abs_timeout)
{
  REAL_FUNC_PASSTHROUGH(mq_timedsend)(
      mqdes, msg_ptr, msg_len, msg_prio, abs_timeout);
}
...
{
  REAL_FUNC_PASSTHROUGH_TYPED(void*, mmap)(
      addr, length, prot, flags, fd, offset);
}

LIB_PRIVATE
void* _real_mmap64(
    void* addr, size_t length, int prot, int flags, int fd, __off64_t offset)
{
  REAL_FUNC_PASSTHROUGH_TYPED(void*, mmap64)(
      addr, length, prot, flags, fd, offset);
}
