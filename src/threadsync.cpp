...
ThreadSync::acquireLocks()
{
  ...
  ASSERT_MUTEX_SUCCESS(DmtcpMutexLock(&libdlLock));
...
ThreadSync::releaseLocks()
{
  ...
  ASSERT_MUTEX_SUCCESS(DmtcpMutexUnlock(&libdlLock));
}
...
ThreadSync::resetLocks(bool resetPresuspendEventHookLock)
{
  ...
  ThreadCoreInfo_ResetWrapperLockCount(&thread->core);
...
ThreadSync::wrapperExecutionLockLock()
{
  ...
  if (ThreadCoreInfo_GetWrapperLockCount(&thread->core) == 0) {
    // If we don't have a lock, acquire it now.
    ASSERT_RWLOCK_SUCCESS(DmtcpRWLockRdLock(&_wrapperExecutionLock));
  }
  ThreadCoreInfo_IncrementWrapperLockCount(&thread->core);
...
ThreadSync::wrapperExecutionLockUnlockForNewThread(Thread *thread)
{
  ...
  ASSERT_RWLOCK_SUCCESS(DmtcpRWLockUnlock(&_wrapperExecutionLock));
...
ThreadSync::wrapperExecutionLockLockExcl()
{
  ...
  ASSERT_RWLOCK_SUCCESS(DmtcpRWLockWrLock(&_wrapperExecutionLock));
  ThreadCoreInfo_IncrementWrapperLockCount(&thread->core);
  errno = saved_errno;
}
...
ThreadSync::wrapperExecutionLockUnlock()
{
  ...
  ASSERT(ThreadCoreInfo_GetWrapperLockCount(&thread->core) != 0,
         ...
         ThreadCoreInfo_GetWrapperLockCount(&thread->core));
  ThreadCoreInfo_DecrementWrapperLockCount(&thread->core);
...
ThreadSync::presuspendEventHookLockLock()
{
  ...
  ASSERT_MUTEX_SUCCESS(DmtcpMutexLock(&presuspendEventHookLock));
}
...
ThreadSync::presuspendEventHookLockUnlock()
{
  ...
  ASSERT_MUTEX_SUCCESS(DmtcpMutexUnlock(&presuspendEventHookLock));
}
