 ...
 */
static int test_use_compression(char* compressor,
                                char* command,
                                char* path,
                                int def)
{
  ...
  if (do_we_compress == NULL) do_we_compress = default_val;

  char* endptr;
  strtol(do_we_compress, &endptr, 0);
  if (*do_we_compress == '\0' || *endptr != '\0') {
    JWARNING(false)(evar)(do_we_compress).Text(
        "Compression env var not defined as a number."
        "  Checkpoint image will not be compressed.");
    do_we_compress = const_cast<char*>("0");
  }

  if (0 == strcmp(do_we_compress, "0")) return 0;
...
#ifdef HBICT_DELTACOMP
static int open_ckpt_to_write_hbict(int fd,
                                    int pipe_fds[2],
                                    char* hbict_path,
                                    char* gzip_path)
{
  char* hbict_args[] = {
      const_cast<char*>("hbict"), const_cast<char*>("-a"), NULL, NULL};
...
{
  char* gzip_args[] = {const_cast<char*>("gzip"),
                       const_cast<char*>("-1"),
                       const_cast<char*>("-"),
                       NULL};
                                      ...
                                      int* fdCkptFileOnDisk)
{
  ...
  *fdCkptFileOnDisk = fd; /* if use_compression, fd will be reset to pipe */
  ...
  use_gzip_compression =
      test_use_compression(const_cast<char*>("GZIP"), gzip_cmd, gzip_path, 1);
...
/* 2b. Test if using HBICT compression */
  ...
  use_deltacompression = test_use_compression(
      const_cast<char*>("HBICT"), hbict_cmd, hbict_path, 1);
#endif
...
int open_ckpt_to_write(int fd, int pipe_fds[2], char** extcomp_args)
{
  pid_t cpid;

  cpid = _real_sys_fork();
  if (cpid == -1) {
    JWARNING(false)(extcomp_args[0])(JASSERT_ERRNO).Text(
        "WARNING: error forking child process. Compression won't be used");
...
static int open_ckpt_to_read(const char* filename)
{
  ...
  static const char* hbict_args[] = {
      const_cast<char*>("hbict"), const_cast<char*>("-r"), NULL};
#endif
  pid_t cpid;
...
void CkptSerializer::createCkptDir()
{
  ...
  (JASSERT_ERRNO)(ckptDir).Text("Error creating checkpoint directory");
...

{
  ...
  fd = perform_open_ckpt_image_fd(
      tempCkptFilename.c_str(), &use_compression, &fdCkptFileOnDisk);
