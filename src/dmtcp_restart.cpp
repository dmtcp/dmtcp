...
static void runMtcpRestart(int is32bitElf, int fd, ProcessInfo* pInfo)
{
  ...
  if (is32bitElf) {
    mtcprestart =
        jalib::Filesystem::FindHelperUtility("mtcp_restart-32", is32bitElf);
  }
...
int main(int argc, char** argv)
{
  ...
  JASSERT(independentProcessTreeRoots.size() > 0).Text(
      "There must atleast one process tree which doesn't have a different "
  ...
  JASSERT(t->pid() != 0);
  JASSERT(!t->noCoordinator() || allowedModes == COORD_ANY).Text(
      "Process had no coordinator prior to checkpoint; but either --join or"
