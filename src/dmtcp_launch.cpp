...
static const char* theUsage =
    ...
    "              Hostname where dmtcp_coordinator is run (default: "
    "localhost)\n"
    "  -p, --port PORT_NUM (environment variable DMTCP_PORT)\n"
    "              Port where dmtcp_coordinator is run (default: 7779)\n"
    "  --port-file FILENAME\n"
    "              File to write listener port number.\n"
    "              (Useful with '--port 0', which is used to assign a random "
    "port)\n"
    "  -j, --join\n"
    ...
    "              already exists on the given port. The port can be "
    "specified\n"
    "              with --port, or with environment variable DMTCP_PORT.  If "
    "no\n"
    "              port is specified, start coordinator at a random port "
    "(same\n"
    ...
    "var,\n"
    "              use default value set in dmtcp_coordinator or "
    "dmtcp_command.\n"
    "              Not allowed if --join is specified\n"
    "\n"
    "Checkpoint image generation:\n"
    "  --gzip, --no-gzip, (environment variable DMTCP_GZIP=[01])\n"
    "              Enable/disable compression of checkpoint images (default: "
    "1)\n"
    "              WARNING:  gzip adds seconds.  Without gzip, ckpt is often < "
    "1 s\n"
    ...
    "  --batch-queue, --rm\n"
    "              Enable support for resource managers (Torque PBS and "
    "SLURM).\n"
    "              (default: disabled)\n"
    "  --ptrace    \n"
    ...
    "              (default: disabled)\n"
    "  --modify-env\n"
    "              Update environment variables based on the environment on "
    "the \n"
    ...
    "              (default: $TMDPIR/dmtcp-$USER@$HOST or "
    "/tmp/dmtcp-$USER@$HOST)\n"
  ...
  while (true) {
    ...
    if ((s == "--help") && argc == 1) {
      ...
      shift;
               ...
               (s == "--mtcp-checkpoint-signal" || s == "--ckpt-signal")) {
...
int main(int argc, char** argv)
{
  ...
  if (!getenv(ENV_VAR_QUIET)) setenv(ENV_VAR_QUIET, "0", 0);
  ...
  << "*** WARNING:  RLIMIT_STACK > 1/4 GB.  This causes each thread to"
     "\n***  receive a 1/4 GB stack segment.  Checkpoint/restart will be "
     "slow,"
     ...
     "\n*** prior to using DMTCP.  (This will be fixed in the future, "
     "when"
  ...
  if (Util::elfType(argv[0], &isElf, &is32bitElf) == -1) {
    ...
    JASSERT_STDERR << "*** ERROR:  Executable to run w/ DMTCP appears not to "
                      "be readable,\n"
  ...
  CoordinatorAPI::instance().connectToCoordOnStartup(
      allowedModes, argv[0], &compId, &coordInfo, &localIPAddr);
  ...
  SharedData::initialize(
      Util::getTmpDir().c_str(), &compId, &coordInfo, &localIPAddr);
  ...
  JASSERT_STDERR << "ERROR: Failed to exec(\"" << argv[0]
                 << "\"): " << JASSERT_ERRNO << "\n"
                 << "Perhaps it is not in your $PATH?\n"
                 << "See `dmtcp_launch --help` for usage.\n";
...
static int testJava(char** argv)
{
  static const char* theJavaWarning =
      ...
      "****  for its heap on startup.  As of DMTCP version 1.2.4, DMTCP "
      "_does_\n"
      ...
      "****  use the -Xmx flag for a smaller heap:  e.g.  java -Xmx64M "
      "javaApp\n"
      "****  (Invoke dmtcp_launch with --quiet to avoid this msg.)\n\n";

  if (getenv(ENV_VAR_QUIET) != NULL && strcmp(getenv(ENV_VAR_QUIET), "0") != 0)
    return 0;
  ...
  if ((fd = open("/proc/meminfo", O_RDONLY)) != -1 &&
      ...
      atol(buf + sizeof(meminfoPrefix)) >
          17000000) /* units of kB : mem > 4 GB */
...
static bool testSetuid(const char* filename)
{
  if (Util::isSetuid(filename) && strcmp(filename, "screen") != 0 &&
      strstr(filename, "/screen") == NULL) {
    static const char* theSetuidWarning =
        "\n**** WARNING:  This process has the setuid or setgid bit set.  This "
        "is\n"
        ...
        "***  compile your own private copy, without using setuid "
        "permission.\n\n";
...
static void setLDPreloadLibs(bool is32bitElf)
{
  ...
  if (is32bitElf) {
    ...
    JWARNING(libdmtcp != "libdmtcp.so").Text(
        "You appear to be checkpointing a 32-bit target under 64-bit Linux.\n"
