...
static const char *theUsage =
  ...
  "DMTCP_COORD_PORT.\n"
  "              If no port is specified, start coordinator at a random "
  "port\n"
  ...
  "  -i, --interval SECONDS (environment variable "
  ...
  "              Directory to store temporary files (default: $TMDPIR or "
  "/tmp)\n"
...
testStaticallyLinked(const char *pathname)
{
  if (Util::isStaticallyLinked(pathname)) {
    JASSERT_STDERR
      << "*** WARNING:  " ELF_INTERPRETER " --verify " << pathname
      << " returns\n"
      << "***  nonzero status.\n"
