...
{
  JASSERT(strcmp(DMTCP_MAGIC_STRING, _magicBits) == 0)(_magicBits).Text(
...
{
  memset(_magicBits, 0, sizeof(_magicBits));
}
...
ostream& dmtcp::operator<<(dmtcp::ostream& o, const WorkerState& s)
{
  o << "WorkerState::";
  switch (s.value()) {
#define OSHIFTPRINTF(name) \
  case WorkerState::name:  \
    o << #name;            \
    break;
...
const char* WorkerState::toString() const
{
  switch (_state) {
    ...
    case SUSPENDED:
      return "SUSPENDED";
    case FD_LEADER_ELECTION:
      return "FD_LEADER_ELECTION";
    case NAME_SERVICE_DATA_REGISTERED:
      return "NAME_SERVICE_DATA_REGISTERED";
    case DONE_QUERYING:
      return "DONE_QUERYING";
    case DRAINED:
      return "DRAINED";
    case RESTARTING:
      return "RESTARTING";
