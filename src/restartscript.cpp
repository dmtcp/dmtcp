...
static const char *multiHostProcessing =
  ...
  "$noStrictChecking $localhost_ckpt_files_group\n"
  "fi\n\n"
            ...
            const map<string, vector<string> > &restartFilenames)
{
  ...
  size_t numPeers;
  for (host = restartFilenames.begin(); host != restartFilenames.end();
       host++) {
  ...
  fclose(fp);
  {
    ...
    JWARNING(
      symlinkat(basename(uniqueFilename.c_str()), dirfd, filename.c_str()) == 0)
    (JASSERT_ERRNO);
