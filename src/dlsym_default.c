...
hash_first(const char *name, Elf32_Word *hash_table, int use_gnu_hash)
{
  if (use_gnu_hash) {
    ...
    uint32_t *buckets = (uint32_t *)((char *)hash_table + 4 * sizeof(uint32_t) +
                                     maskwords * sizeof(long unsigned int));
...
hash_next(Elf32_Word index, Elf32_Word *hash_table, int use_gnu_hash)
{
  if (use_gnu_hash) {
    ...
    uint32_t maskwords = ((uint32_t *)hash_table)[2];
    uint32_t *hashval =
      (uint32_t *)((char *)hash_table + 4 * sizeof(uint32_t) /* sizeof header */
                   +
                   maskwords *
                     sizeof(long unsigned int) /* sizeof Bloom filter */
...
version_name(ElfW(Word) version_ndx, dt_tag *tags)
{
  ...
  for (prev = NULL, cur = (ElfW(Verdef) *)(tags->base_addr +
                                           (unsigned long int)(tags->verdef));
...
void *
dlsym_default_internal(void *handle, const char *symbol)
{
  dt_tag tags;
