...
static char* version_name(ElfW(Word) version_ndx, dt_tag* tags)
{
  ...
  if (version_ndx & (1 << 15)) version_ndx -= (1 << 15);
  // Walk the list of all versions.
  for (
      prev = NULL,
     cur = (ElfW(Verdef)*)(tags->base_addr + (unsigned long int)(tags->verdef));
...
static void get_dt_tags(void* handle, dt_tag* tags)
{
  ...
  for (cur_dyn = dyn; cur_dyn->d_tag != DT_NULL; cur_dyn++) {
    if (cur_dyn->d_tag == DT_VERSYM) tags->versym = (void*)cur_dyn->d_un.d_ptr;
    if (cur_dyn->d_tag == DT_VERDEF) tags->verdef = (void*)cur_dyn->d_un.d_ptr;
    ...
    if (cur_dyn->d_tag == DT_SYMTAB) tags->symtab = (void*)cur_dyn->d_un.d_ptr;
    if (cur_dyn->d_tag == DT_HASH) tags->hash = (void*)cur_dyn->d_un.d_ptr;
    ...
    //  first_ext_def = (ElfW(Word))cur_dyn->d_un.d_val;  // first dynsym
    //  entry??
  }
}
...
//   info from dynamic section.
  ...
  for (i = hash_first(symbol, tags.hash); i != STN_UNDEF;
    ...
    if (tags.symtab[i].st_name == 0 || tags.symtab[i].st_value == 0) continue;
  ...
  if (!default_symbol_index) {
    printf(
        "ERROR:  No default symbol version found for %s.\n"
