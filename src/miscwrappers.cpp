...
/*
 * FIXME: Add wrapper for dup2 and dup3 to detect if the newfd is a protected
fd.
...
syscall(long sys_num, ...)
{
  ...
  switch (sys_num) {
    ...
    case SYS_execve: {
      SYSCALL_GET_ARGS_3(const char *, filename, char *const *, argv,
                         char *const *, envp);
    ...
    case SYS_connect: {
      SYSCALL_GET_ARGS_3(int, sockfd, const struct sockaddr *, addr, socklen_t,
                         addrlen);
    ...
    case SYS_accept: {
      SYSCALL_GET_ARGS_3(int, sockfd, struct sockaddr *, addr, socklen_t *,
                         addrlen);
    ...
    case SYS_accept4: {
      SYSCALL_GET_ARGS_4(int, sockfd, struct sockaddr *, addr, socklen_t *,
                         addrlen, int, flags);
    ...
    // __GLIBC_PREREQ(2, 10)
    case SYS_setsockopt: {
      SYSCALL_GET_ARGS_5(int, s, int, level, int, optname, const void *, optval,
                         socklen_t, optlen);
...
extern "C" int vsnprintf(char *str, size_t size, const char *format, va_list
ap);
*/
