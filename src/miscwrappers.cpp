...
extern "C" void exit(int status)
{
  ...
  _real_exit(status);
  for (;;)
    ; // Without this, gcc emits warning:  `noreturn' fnc does return
}
                       ...
                       struct rusage* rusage)
{
  ...
  if (retval > 0 && (WIFEXITED(*(int*)status) || WIFSIGNALED(*(int*)status))) {
...
extern "C" SYSCALL_ARG_RET_TYPE syscall(SYSCALL_ARG_RET_TYPE sys_num, ...)
{
  ...
  switch (sys_num) {
    ...
    case SYS_rt_sigtimedwait: {
      SYSCALL_GET_ARGS_3(const sigset_t*,
                         set,
                         siginfo_t*,
                         info,
