...
typedef int (*clone_fptr_t)(int (*fn)(void *arg),
                            void *child_stack,
                            int flags,
                            void *arg,
                            int *parent_tidptr,
...
syscall(long sys_num, ...)
{
  ...
  switch (sys_num) {
    ...
    case SYS_waitid: {
#if 0
      SYSCALL_GET_ARGS_4(idtype_t, idtype, id_t, id, siginfo_t*, infop,
                         int, options);
#endif
      SYSCALL_GET_ARGS_4(int, idtype, id_t, id, siginfo_t *, infop, int,
                         options);
    ...
    case SYS_process_vm_readv: {
      pid_t realPid;
      DMTCP_PLUGIN_DISABLE_CKPT();
      SYSCALL_GET_ARGS_6(pid_t, pid, const struct iovec *, local_iov,
                         unsigned long, liovcnt, const struct iovec *,
                         remote_iov, unsigned long, riovcnt, unsigned long,
                         flags);
      realPid = VIRTUAL_TO_REAL_PID(pid);
      ret = _real_syscall(SYS_process_vm_readv, realPid, local_iov, liovcnt,
    ...
    case SYS_process_vm_writev: {
      ...
      SYSCALL_GET_ARGS_6(pid_t, pid, const struct iovec *, local_iov,
                         ...
                         flags);
      realPid = VIRTUAL_TO_REAL_PID(pid);
      ret = _real_syscall(SYS_process_vm_writev, realPid, local_iov, liovcnt,
                          remote_iov, riovcnt, flags);
      DMTCP_PLUGIN_ENABLE_CKPT();
      break;
    }
                 ...
                 unsigned long flags)
{
  ...
  DMTCP_PLUGIN_DISABLE_CKPT();
  realPid = VIRTUAL_TO_REAL_PID(pid);
  ret = _real_process_vm_readv(realPid, local_iov, liovcnt, remote_iov, riovcnt,
                               flags);
  DMTCP_PLUGIN_ENABLE_CKPT();
                  ...
                  unsigned long flags)
{
  ...
  DMTCP_PLUGIN_DISABLE_CKPT();
  realPid = VIRTUAL_TO_REAL_PID(pid);
  ret = _real_process_vm_writev(realPid, local_iov, liovcnt, remote_iov,
                                riovcnt, flags);
  DMTCP_PLUGIN_ENABLE_CKPT();
