  ...
  while (readyToRead(fd) && rc <= 0) {
    ...
    *(PtyHdr *)buf = rc; // Record the number read in header
    if (rc >= (ssize_t)(maxCount - sizeof(PtyHdr))) {
      rc = -1;
      errno = E2BIG; // Invoke new errno for buf size not large enough
    }
...
PtyConnection::PtyConnection(
  int fd, const char *path, int flags, mode_t mode, int type)
  : Connection(PTY), _flags(flags), _mode(mode), _preExistingCTTY(false)
{
...
PtyConnection::refill(bool isRestart)
{
  ...
  if (_type == PTY_SLAVE || _type == PTY_BSD_SLAVE) {
    ...
    JASSERT(tempfd >= 0)
    (_virtPtsName)(_ptsName)(JASSERT_ERRNO).Text("Error Opening PTS");
  ...
  if (_type == PTY_DEV_TTY) {
    ...
    JASSERT(tempfd >= 0)
    (tempfd)(JASSERT_ERRNO).Text("Error opening controlling terminal /dev/tty");
  ...
  switch (_type) {
    ...
    case PTY_PARENT_CTTY: {
      ...
      if (controllingTty.length() > 0 &&
        ...
        JASSERT(tempfd >= 0)
        (tempfd)(_fcntlFlags)(controllingTty)(JASSERT_ERRNO)
    ...
    case PTY_MASTER: {
      ...
      JASSERT(tempfd >= 0)
      (tempfd)(JASSERT_ERRNO).Text("Error Opening /dev/ptmx");
