...
extern "C" char *
__realpath(const char *path, char *resolved_path)
{
  return realpath(path, resolved_path);
}

extern "C" char *
__realpath_chk(const char *path, char *resolved_path, size_t resolved_len)
{
  return realpath(path, resolved_path);
}
...
ptsname_r_work(int fd, char *buf, size_t buflen)
{
  ...
  if (virtPtsName.length() >= buflen) {
    JWARNING(false)
    (virtPtsName)(virtPtsName.length())(buflen).Text(
      "fake ptsname() too long for user buffer");
    errno = ERANGE;
    return -1;
  }
...
getpt()
{
  ...
  if (fd >= 0 && dmtcp_is_running_state()) {
    PtyConnection *c = new PtyConnection(fd, "/dev/ptmx", O_RDWR | O_NOCTTY, -1,
                                         PtyConnection::PTY_MASTER);
    PtyConnList::instance().add(fd, c);
  }
  DMTCP_PLUGIN_ENABLE_CKPT();
  return fd;
...
posix_openpt(int flags)
{
  ...
  if (fd >= 0 && dmtcp_is_running_state()) {
    PtyConnection *c =
      new PtyConnection(fd, "/dev/ptmx", flags, -1, PtyConnection::PTY_MASTER);
    PtyConnList::instance().add(fd, c);
  }
  DMTCP_PLUGIN_ENABLE_CKPT();
  return fd;
