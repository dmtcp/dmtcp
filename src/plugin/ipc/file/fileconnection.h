...
public:
  ...
  StdioConnection(int fd) : Connection(STDIO + fd)
  {
    ...
    JASSERT(jalib::Between(0, fd, 2))
    (fd).Text("invalid fd for StdioConnection");
  }
...
public:
  ...
  FileConnection(const string &path,
    ...
    : Connection(FILE), _path(path), _fileAlreadyExists(false), _flags(flags)
