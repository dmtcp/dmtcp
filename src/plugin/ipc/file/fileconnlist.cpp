...
FileConnList::prepareShmList()
{
  ...
  while (procSelfMaps.getNextArea(&area)) {
    if ((area.flags & MAP_SHARED) && area.prot != 0) {
      ...
      if (Util::isNscdArea(area) || Util::isIBShmArea(area) ||
...
FileConnList::recreateShmFileAndMap(const ProcMapsArea &area)
{
  ...
  JASSERT(Util::createDirectoryTree(area.name))
  (area.name).Text("Unable to create directory in File Path");
...
FileConnList::restoreShmArea(const ProcMapsArea &area, int fd)
{
  ...
  (area.flags)(area.prot)(JASSERT_ERRNO).Text("mmap failed");
  _real_close(fd);
}
