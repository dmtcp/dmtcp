    ...
    case PTY_MASTER:
      ...
      JASSERT((strlen(buf) + strlen("v")) <= 20)
        .Text("string /dev/pts/<n> too long, can not be virtualized."
              "One possible workaround here is to replace the string"
...
void PtyConnection::drain()
{
  ...
  saveOptions();
  /* TODO: why does tcgetattr fail for background process in 'screen' test?
   *    */
   ...
   * tcgetpgrp is not documented in the man page, and arguably is either a bug
   * in the documentation or in the kernel for version 4.4 and glibc-2.21
   */
  if (_type == PTY_MASTER) {
...
void FileConnection::postRestart()
{
  ...
  // In DMTCP-3.0, this use of Util::dupFds() was replaced by:
  //   restoreDupFds(tempfd);
  // in order to hide "_fds" from the reader, and make this extra hard to
  // understand!
  Util::dupFds(tempfd, _fds);
}

bool FileConnection::checkDup(int fd, const char *npath)
{
  bool retVal = false;
