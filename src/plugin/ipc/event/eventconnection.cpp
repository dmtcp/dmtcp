...
EpollConnection::refill(bool isRestart)
{
  ...
  if (isRestart) {
    ...
    for (; fevt != _fdToEvent.end(); fevt++) {
      ...
      int ret =
        _real_epoll_ctl(_fds[0], EPOLL_CTL_ADD, fevt->first, &(fevt->second));
      JWARNING(ret == 0)
      (_fds[0])(ret)(strerror(errno)).Text("Error in restoring options");
    }
  }
}
...
EpollConnection::onCTL(int op, int fd, struct epoll_event *event)
{
  JASSERT(((op == EPOLL_CTL_MOD || op == EPOLL_CTL_ADD) && event != NULL) ||
          op == EPOLL_CTL_DEL)
  (epollType())(id()).Text("Passing a NULL event! HUH!");
...
InotifyConnection::refill(bool isRestart)
{
  ...
  if (isRestart) {
    ...
    for (int i = 0; i < num_of_descriptors; i++) {
      if (true ==
          descriptor.get_descriptor(i, INOTIFY_ADD_WATCH_DESCRIPTOR,
                                    &watch_descriptor)) {
        int old_wd = watch_descriptor.add_watch.watch_descriptor;

        int new_wd = _real_inotify_add_watch(
