...
public:
  ...
  EpollConnection(int size, int type = EPOLL_CREATE)
    : Connection(EPOLL), _type(type), _size(size)
  {
...
public:
  inline EventFdConnection(unsigned int initval, int flags)
    : Connection(EVENTFD), _initval(initval), _flags(flags)
  {
...
public:
  inline SignalFdConnection(int signalfd, const sigset_t *mask, int flags)
    : Connection(SIGNALFD), signlfd(signalfd), _flags(flags)
  {
    if (mask != NULL) {
      _mask = *mask;
    } else {
...
public:
  ...
  inline InotifyConnection(int flags)
    : Connection(INOTIFY), _flags(flags), _state(INOTIFY_CREATE)
  {
