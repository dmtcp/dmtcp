...
_makeDeadSocket(const char *refillData = NULL, ssize_t len = -1)
{
  ...
  JASSERT(sp[0] >= 0 && sp[1] >= 0)(sp[0])(sp[1]).Text("socketpair() failed");
  if (refillData != NULL) {
...
SocketConnection::restoreSocketOptions(vector<int> &fds)
{
  ...
  for (levelIterator lvl = _sockOptions.begin(); lvl != _sockOptions.end();
       ++lvl) {
    for (optionIterator opt = lvl->second.begin(); opt != lvl->second.end();
         ++opt) {
      ...
      JASSERT(ret == 0)
      (JASSERT_ERRNO)(fds[0])(lvl->first)(opt->first)(opt->second.size())
    ...
    _remotePeerId(ConnectionIdentifier::null())
{
  if (domain != -1) {
    ...
    if ((type & 077) == SOCK_DGRAM) {
      JWARNING(false)
      (type).Text("Datagram Sockets not supported. "
  ...
  if (saddr->sa_family == AF_INET) {
    ...
    for (size_t i = 0; blacklistedRemotePorts[i] != -1; i++) {
      if (ntohs(addr->sin_port) == blacklistedRemotePorts[i]) {
        JTRACE("LDAP port found")
        (ntohs(addr->sin_port))(blacklistedRemotePorts[0])(
          blacklistedRemotePorts[1]);
        return true;
      }
    }
...
TcpConnection::onBind(const struct sockaddr *addr, socklen_t len)
{
  ...
  if (_sockDomain == AF_UNIX && addr != NULL) {
    JASSERT(len <= sizeof _bindAddr)
    (len)(sizeof _bindAddr).Text("That is one huge sockaddr buddy.");
    _bindAddrlen = len;
    ...
    // using getsockname.
    JASSERT(
      getsockname(_fds[0], (struct sockaddr *)&_bindAddr, &_bindAddrlen) == 0)
    (JASSERT_ERRNO);
  }
  _type = TCP_BIND;
}
...
TcpConnection::onListen(int backlog)
{
  ...
  JASSERT(_type == TCP_BIND)
  (_type)(id()).Text("Listening on a non-bind()ed socket????");
                         ...
                         bool connectInProgress)
{
  ...
  JWARNING(_type == TCP_CREATED || _type == TCP_BIND)
  (_type)(id()).Text("Connecting with an in-use socket????");
...
TcpConnection::TcpConnection(const TcpConnection &parent,
  ...
  : Connection(TCP_ACCEPT),
    SocketConnection(
      parent._sockDomain, parent._sockType, parent._sockProtocol),
    _listenBacklog(-1),
    _bindAddrlen(0),
...
TcpConnection::drain()
{
  ...
  if (_type == TCP_CONNECT_IN_PROGRESS) {
    ...
    if (retval == -1) {
      ...
      JWARNING(false)
      (_fds[0]).Text(
        "connect() returned EINPROGRESS.  Socket still not writable\n"
  ...
  switch (_type) {
    ...
    case TCP_LISTEN:
      ...
      break;
    case TCP_BIND:
      JWARNING(_type != TCP_BIND)
      (_fds[0]).Text("If there are pending connections on this socket,\n"
  ...
  switch (_type) {
    ...
    case TCP_LISTEN:
      ...
      if (_sockDomain == AF_INET6) {
        ...
        typedef map<int64_t, map<int64_t, jalib::JBuffer> >::iterator
          levelIterator;
        typedef map<int64_t, jalib::JBuffer>::iterator optionIterator;
              ...
              if (opt->first == IPV6_V6ONLY) {
                ...
                int ret =
                  _real_setsockopt(_fds[0], lvl->first, opt->first,
                                   opt->second.buffer(), opt->second.size());
                JASSERT(ret == 0)
                (JASSERT_ERRNO)(_fds[0])(lvl->first)(opt->first)(
                  opt->second.buffer())(opt->second.size())
                  .Text("Restoring setsockopt failed.");
              }
            }
    ...
    case TCP_CONNECT:
      ...
      Util::dupFds(fd, _fds);
      if (_bindAddrlen != 0) {
        JWARNING(_real_bind(_fds[0], (sockaddr *)&_bindAddr, _bindAddrlen) !=
                 -1)
        (JASSERT_ERRNO);
      }
...
TcpConnection::recvHandshake(int remotefd, const ConnectionIdentifier &coordId)
{
  ...
  JASSERT(msg.coordId == coordId)
  (msg.coordId)(coordId).Text(
    "Peer has a different dmtcp_coordinator than us!\n"
    "  It must be the same.");
  ...
  if (_remotePeerId.isNull()) {
    ...
    (_remotePeerId)(msg.from).Text(
      "Read handshake with a different 'from' field"
...
RawSocketConnection::RawSocketConnection(int domain, int type, int protocol)
  : Connection(RAW), SocketConnection(domain, type, protocol)
{
  JASSERT(type == -1 || (type & SOCK_RAW));
  JASSERT(domain == -1 || domain == AF_NETLINK)
  (domain).Text("Only Netlink raw socket supported");
