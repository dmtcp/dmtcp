...
int SigInfo::ckptSignal()
{
  return STOPSIGNAL;
}
extern "C" int SigInfo_ckptSignal()
{
  return STOPSIGNAL;
}
...
void SigInfo::setupCkptSigHandler(sighandler_t handler)
{
  ...
  if (!init) {
    ...
    if (tmp == NULL) {
      ...
      if ((errno != 0) || (tmp == endp)) {
        JWARNING(false)(getenv("DMTCP_SIGCKPT"))(CKPT_SIGNAL).Text(
            "Your chosen SIGCKPT does not translate to a number, and cannot be"
            "used.  Default signal will be used instead");
        STOPSIGNAL = CKPT_SIGNAL;
      } else if (STOPSIGNAL < 1 || STOPSIGNAL > 31) {
        JNOTE(
            "Your chosen SIGCKPT is not a valid signal, and cannot be used."
  ...
  if ((old_act.sa_handler != SIG_IGN) && (old_act.sa_handler != SIG_DFL) &&
    ...
    JASSERT(false)(STOPSIGNAL)(old_act.sa_handler).Text(
...
void SigInfo::restoreSigHandlers()
{
  ...
  for (sig = SIGRTMAX; sig > 0; --sig) {
    ...
    JASSERT(_real_sigaction(sig, &sigactions[sig], NULL) == 0 ||
            errno == EINVAL)
