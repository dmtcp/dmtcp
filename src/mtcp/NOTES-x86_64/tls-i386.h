...
#else // if defined NEED_DL_SYSINFO
#define INIT_SYSINFO
#endif // if defined NEED_DL_SYSINFO
...
#define TLS_INIT_TP(thrdescr, secondcall)                                   \
  ({                                                                        \
    ...
    _segdescr.vals[3] = 0x51;                                               \
                                                                            \
    /* Install the TLS.  */                                                 \
    asm volatile(TLS_LOAD_EBX "int $0x80\n\t" TLS_LOAD_EBX                  \
                 : "=a"(_result), "=m"(_segdescr.desc.entry_number)         \
                 : "0"(__NR_set_thread_area), TLS_EBX_ARG(&_segdescr.desc), \
                   "m"(_segdescr.desc));                                    \
                                                                            \
    if (_result == 0) {                                                     \
  ...
  ({                                                                      \
    if (sizeof(descr->member) == 1) {                                     \
      asm volatile("movb %b0,%%gs:%P1"                                    \
                   :                                                      \
                   : "iq"(value), "i"(offsetof(struct pthread, member))); \
    } else if (sizeof(descr->member) == 4) {                              \
      asm volatile("movl %0,%%gs:%P1"                                     \
                   :                                                      \
                   : "ir"(value), "i"(offsetof(struct pthread, member))); \
    } else {                                                              \
      ...
      asm volatile("movl %%eax,%%gs:%P1\n\t"                              \
                   "movl %%edx,%%gs:%P2"                                  \
                   :                                                      \
                   : "A"(value), "i"(offsetof(struct pthread, member)),   \
                     "i"(offsetof(struct pthread, member) + 4));          \
    }                                                                     \
  })
  ...
  (void)({                                                                     \
    if (sizeof((descr)->member) == 4) {                                        \
      asm volatile(LOCK_PREFIX "orl %1, %%gs:%P0"                              \
                   :                                                           \
                   : "i"(offsetof(struct pthread, member)), "ir"(1 << (bit))); \
    } else {                                                                   \
      /* Not necessary for other sizes in the moment.  */                      \
      abort();                                                                 \
    }                                                                          \
