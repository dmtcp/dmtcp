  ...
  ({                                                                      \
    ...
    if (sizeof(__value) == 1)                                             \
      asm volatile(                                                       \
          "movb %%gs:%P2(%3),%b0"                                         \
          : "=q"(__value)                                                 \
          : "0"(0), "i"(offsetof(struct pthread, member[0])), "r"(idx));  \
    else if (sizeof(__value) == 4)                                        \
  ...
  ({                                                                       \
    if (sizeof(descr->member[0]) == 1)                                     \
      asm volatile(                                                        \
          "movb %b0,%%gs:%P1(%2)"                                          \
          :                                                                \
          : "iq"(value), "i"(offsetof(struct pthread, member)), "r"(idx)); \
    else if (sizeof(descr->member[0]) == 4)                                \
      asm volatile(                                                        \
          "movl %0,%%gs:%P1(,%2,4)"                                        \
          :                                                                \
          : "ir"(value), "i"(offsetof(struct pthread, member)), "r"(idx)); \
    else {                                                                 \
      ...
      asm volatile(                                                        \
          ...
          :                                                                \
          : "A"(value), "i"(offsetof(struct pthread, member)), "r"(idx));  \
    }                                                                      \
  })
...
#define THREAD_ATOMIC_CMPXCHG_VAL(descr, member, newval, oldval)             \
  ({                                                                         \
    ...
    if (sizeof(descr->member) == 4)                                          \
      asm volatile(                                                          \
          LOCK_PREFIX "cmpxchgl %2, %%gs:%P3"                                \
          : "=a"(__ret)                                                      \
          : "0"(__old), "r"(newval), "i"(offsetof(struct pthread, member))); \
    else                                                                     \
  ...
  (void)({                                                                     \
    if (sizeof((descr)->member) == 4)                                          \
      asm volatile(LOCK_PREFIX                                                 \
                   "orl %1, %%gs:%P0" ::"i"(offsetof(struct pthread, member)), \
                   "ir"(1 << (bit)));                                          \
    else                                                                       \
      /* Not necessary for other sizes in the moment.  */                      \
      abort();                                                                 \
