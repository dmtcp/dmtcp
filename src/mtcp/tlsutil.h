...
#ifdef __i386__
#define tls_get_thread_area(arg, myinfo_gs) mtcp_sys_get_thread_area(arg)
#define tls_set_thread_area(arg, myinfo_gs) mtcp_sys_set_thread_area(arg)
...
#define tls_set_thread_area(uinfo, myinfo_gs)                          \
  (arch_prctl(ARCH_SET_FS, *(unsigned long int *)&(                    \
                             ((struct user_desc *)uinfo)->base_addr)), \
...
#define tls_set_thread_area(uinfo, myinfo_gs)                         \
  (myinfo_gs =                                                        \
     *(unsigned long int *)&(((struct user_desc *)uinfo)->base_addr), \
   (mtcp_sys_kernel_set_tls(myinfo_gs + 1216),                        \
    0) /* 0 return value at end means success */)
#endif /* end __arm__ */
