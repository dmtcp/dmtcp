...
#define COPY_BINARY
  ...
  snprintf(newFilename, PATH_MAX, "%s/%s", dmtcp_get_tmpdir(),
           jalib::Filesystem::BaseName(realFilename).c_str());

  snprintf(cpCmdBuf, sizeof(cpCmdBuf), "/bin/cp %s %s", realFilename,
           newFilename);

  // Remove any stale copy, just in case it's not right.
  JASSERT(unlink(newFilename) == 0 || errno == ENOENT)(newFilename);

  JASSERT(safeSystem(cpCmdBuf) == 0)
  (cpCmdBuf).Text("call to system(cpCmdBuf) failed");
  ...
  *argvPtr = (char **)(cmdBuf + strlen(cmdBuf) + 1); // ... + 1 for '\0'
  ...
  JASSERT(newArgvLen >= origArgvLen + 1)
  (origArgvLen)(newArgvLen)
...
#define ENV_PTR(x) ((char *)(getenv(x) - strlen(x) - 1))
  ...
  if (argvSizeDiff > 0) {
    ...
    strncpy(dummyEnviron, ENV_VAR_DMTCP_DUMMY "=",
