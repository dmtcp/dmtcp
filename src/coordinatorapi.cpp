...
{
static void
coordinatorAPI_EventHook(DmtcpEvent_t event, DmtcpEventData_t *data)
{
  if (CoordinatorAPI::noCoordinator()) {
    return;
  }
  switch (event) {
...
void
CoordinatorAPI::setCoordPort(int port)
{
  _cachedPort = port;
}
...
dmtcp_CoordinatorAPI_PluginDescr()
{
...
void
CoordinatorAPI::resetOnFork(CoordinatorAPI &coordAPI)
{
...
CoordinatorAPI::connectAndSendUserCommand(
  char c, int *coordCmdStatus, int *numPeers, int *isRunning, int *ckptInterval)
{
  char *replyData = NULL;
...
string
CoordinatorAPI::getCoordCkptDir(void)
{
...
void
CoordinatorAPI::updateCoordCkptDir(const char *dir)
{
  if (noCoordinator()) {
    return;
  }
...
void
CoordinatorAPI::sendMsgToCoordinator(DmtcpMessage msg,
...
void
CoordinatorAPI::sendMsgToCoordinator(const DmtcpMessage &msg,
                                     const string &data)
{
...
void
CoordinatorAPI::recvMsgFromCoordinator(DmtcpMessage *msg, void **extraData)
{
  msg->poison();
...
void
CoordinatorAPI::waitForBarrier(const string &barrierId)
{
...
CoordinatorAPI::startNewCoordinator(CoordinatorMode mode)
{
  ...
  JASSERT(strcmp(host, "localhost") == 0 || strcmp(host, "127.0.0.1") == 0 ||
  ...
  errno = 0;
  jalib::JServerSocket coordinatorListenerSocket(jalib::JSockAddr::ANY, port,
                                                 128);
  ...
  if (fork() == 0) {
    ...
    char *args[] = {(char *)coordinator.c_str(), (char *)"--quiet",
                    /* If we wish to also suppress coordinator warnings, call
                       --quiet twice */
                    (char *)"--exit-on-last", modeStr, NULL};
    execv(args[0], args);
    JASSERT(false)
    (coordinator)(JASSERT_ERRNO).Text("exec(dmtcp_coordinator) failed");
  } else {
...
void
CoordinatorAPI::createNewConnToCoord(CoordinatorMode mode)
{
  if (mode & COORD_JOIN) {
    ...
    _coordinatorSocket = createNewSocketToCoordinator(mode);
    JASSERT(_coordinatorSocket != -1)
    (JASSERT_ERRNO).Text("Error connecting to newly started coordinator.");
...
DmtcpMessage
CoordinatorAPI::sendRecvHandshake(DmtcpMessage msg,
                                  string progname,
                                  UniquePid *compId)
{
  if (dmtcp_virtual_to_real_pid) {
  ...
  if (msg.type == DMT_REJECT_NOT_RUNNING) {
    ...
    JASSERT(compId != NULL);
    JASSERT(false)
    (*compId).Text("Connection rejected by the coordinator.\n"
...
void
CoordinatorAPI::connectToCoordOnStartup(CoordinatorMode mode,
                                        string progname,
                                        ...
                                        struct in_addr *localIP)
{
  ...
  *compId = hello_remote.compGroup.upid();
  ...
  JASSERT(getpeername(_coordinatorSocket, (struct sockaddr *)&coordInfo->addr,
                      &coordInfo->addrLen) == 0)
  (JASSERT_ERRNO);
...
void
CoordinatorAPI::createNewConnectionBeforeFork(string &progname)
{
...
void
CoordinatorAPI::connectToCoordOnRestart(CoordinatorMode mode,
                                        string progname,
                                        UniquePid compGroup,
                                        int np,
                                        CoordinatorInfo *coordInfo,
                                        ...
                                        struct in_addr *localIP)
{
  ...
  DmtcpMessage hello_remote =
    sendRecvHandshake(hello_local, progname, &compGroup);
...
int
CoordinatorAPI::sendKeyValPairToCoordinator(const char *id,
                                            const void *key,
                                            uint32_t key_len,
                                            const void *val,
                                            uint32_t val_len)
...
int
CoordinatorAPI::sendQueryToCoordinator(const char *id,
                                       const void *key,
                                       uint32_t key_len,
                                       void *val,
                                       uint32_t *val_len)
                                        ...
                                        struct in_addr *localIP)
{
  ...
  JASSERT(_coordinatorSocket != -1)
  (port)(JASSERT_ERRNO).Text("Failed to create listen socket.");
  updateSockFd();
  ...
  UniquePid coordId =
    UniquePid(INITIAL_VIRTUAL_PID, UniquePid::ThisProcess().hostid(),
...
CoordinatorAPI::waitForCheckpointCommand()
{
  ...
  do {
    ...
    int retval = select(_coordinatorSocket + 1, &rfds, NULL, NULL, timeout);
  ...
  JASSERT(msg.type == DMT_USER_CMD)(msg.type).Text("Unexpected connection.");
...
CoordinatorAPI::noCoordinator()
{
  ...
  if (virtualCoordinator == -1) {
    ...
    int ret = _real_getsockopt(PROTECTED_COORD_FD, SOL_SOCKET, SO_ACCEPTCONN,
                               &optVal, &optLen);
    if (ret == 0 && optVal == 1) {
