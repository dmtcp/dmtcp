...
{
  ASSERT(dmtcp_ckpt_header_has_valid_bootstrap(ckptHdr),
         "checkpoint header is not compatible with this restart binary");
}
...
checkVdsoOffsetMismatch(DmtcpCkptHeader *ckptHdr)
{
  ...
  ASSERT(ckptHdr->getcpu_offset == getcpu_offset,
         "{} checkpoint offset={:#x}, current offset={:#x}",
...
getMtcpArgs(MemRegion restoreBuf)
{
  ...
  restoreBufAddrStr = jalib::XToHexString(restoreBuf.startAddr);
  mtcpArgs.push_back((char *) restoreBufAddrStr.c_str());

  mtcpArgs.push_back((char *) "--restore-buffer-len");
  restoreBufLenStr = jalib::XToHexString(restoreBuf.endAddr - restoreBuf.startAddr);
...
runMtcpRestart(int fd, RestoreTarget *restoreTarget)
{
  ...
  vector<char *> mtcpArgs = getMtcpArgs(restoreTarget->restoreBuf());
...
: argc(argc), argv(argv), binaryName(binaryName), mtcpRestartBinaryName(mtcpRestartBinaryName)
{
  ...
  while (argc > 0) { // ... --restartdir ./ OR ... ckptImg
    ...
    if (s == "--help" && argc == 1) {
      ...
      ASSERT(argv[1] && argv[1][0] >= '1' && argv[1][0] <= '7'
             && argv[1][1] == '\0',
