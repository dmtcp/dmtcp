                       ...
                       struct in_addr *localIPAddr = NULL)
{
  ...
  void *addr =
    _real_mmap((void *)sharedDataHeader, size, PROT_READ | PROT_WRITE,
               MAP_SHARED, PROTECTED_SHM_FD, 0);
  JASSERT(addr != MAP_FAILED)
  (JASSERT_ERRNO).Text("Unable to find shared area.");
  ...
  if (needToInitialize) {
    ...
    Util::lockFile(PROTECTED_SHM_FD);
    if (!Util::strStartsWith(sharedDataHeader->versionStr, SHM_VERSION_STR)) {
      JASSERT(false)
      (sharedDataHeader->versionStr)(SHM_VERSION_STR).Text("Wrong signature");
    }
    Util::unlockFile(PROTECTED_SHM_FD);
  }
