                       ...
                       struct in_addr *localIPAddr)
{
  ...
  ASSERT((coordInfo != NULL && localIPAddr != NULL && compId != NULL) ||
...
SharedData::createVirtualPtyName(const char *real, char *out, uint32_t len)
{
  ...
  Util::lockFile(PROTECTED_SHM_FD);
  ASSERT(sharedDataHeader->nextVirtualPtyId !=
           static_cast<uint64_t>(std::numeric_limits<uint32_t>::max()),
         "shared-data virtual PTY id space is exhausted");
  const uint64_t id = sharedDataHeader->nextVirtualPtyId++;
  string virt = VIRT_PTS_PREFIX_STR + jalib::XToString(id);
...
SharedData::insertPtyNameMap(const char *virt, const char *real)
{
  ...
  Util::lockFile(PROTECTED_SHM_FD);
  ASSERT(sharedDataHeader->numPtyNameMaps < MAX_PTY_NAME_MAPS,
         "shared-data PTY name map is full: count={} max={} virt={} real={}",
         sharedDataHeader->numPtyNameMaps, MAX_PTY_NAME_MAPS, virt, real);
  size_t n = sharedDataHeader->numPtyNameMaps++;
