...
This file records source-backed current behavior and the S01 feasibility boundary for the TSAN concern. It does not claim runtime TSAN success; runtime sanitizer proof is reserved for the M004/R011 experiments named below. [src/dlwrappers.cpp] [src/dmtcpworker.cpp]

## Scope and R002 evidence boundary

R002 needs a source-backed map of the current DMTCP loader stack before design work relies on assumptions about wrapper order. The motivating TSAN scenario for future runtime work is sanitized here as a behavioral claim rather than kept as raw correspondence: DMTCP may need initialization to happen early while wrappers remain late, close to libc. The scenario also describes TSAN's constructor running first, DMTCP initialization being reached through a wrapper, and later DMTCP dlsym activity reaching a TSAN `dl_iterate_phdr` interceptor. The source-backed part of that risk is the repository path where DMTCP's external `dlsym(RTLD_NEXT)` wrapper calls `dl_iterate_phdr`, plus the DMTCP constructor entry used for ordering experiments. [src/dlwrappers.cpp] [src/dmtcpworker.cpp]
...
- The current `RTLD_NEXT` and `dl_iterate_phdr` risk surface, including what consolidation can reduce and what still needs runtime proof. [src/dlwrappers.cpp]
...
The sanitized motivating scenario reports a concrete concern: TSAN's constructor can run before DMTCP's constructor, then TSAN can call `dlsym(RTLD_NEXT, ...)`, DMTCP's `dlsym` wrapper can lead to `dl_iterate_phdr`, and TSAN may intercept that call before DMTCP has a protective `dl_iterate_phdr` wrapper or direct libc bypass. The source-backed risk statement is the DMTCP external `dlsym(RTLD_NEXT)` path that calls `dl_iterate_phdr`; the constructor-order part remains a runtime experiment, not proof that any consolidation design solves the case. [src/dlwrappers.cpp] [src/dmtcpworker.cpp]

## R002 coverage checklist

| R002 prompt | Source-backed answer | S01 status |
|---|---|---|
| Constructor order | `libdmtcp.so` enters at constructor priority 101 through `dmtcp_initialize_entry_point`, calls `dmtcp_initialize`, prepares wrappers, initializes `PluginManager`, restores user-visible preload state, then emits `DMTCP_EVENT_INIT`. TSAN may still have its own earlier constructor when the target is built with TSAN; that ordering remains a runtime experiment. [src/dmtcpworker.cpp] | Mapped; runtime TSAN proof deferred. |
...
| TSAN interaction | The motivating stack hypothesis has `__tsan::Initialize` before DMTCP, then a TSAN interception path reaching DMTCP's `dlsym` wrapper and `___interceptor_dl_iterate_phdr`; the source confirms DMTCP's external `dlsym(RTLD_NEXT)` path calls `dl_iterate_phdr`. [src/dlwrappers.cpp] [src/dmtcpworker.cpp] | Feasible concern; not solved by documentation alone. |
...
| dl_iterate_phdr | The risky direct call is in `dlsym_with_rtld_next`; S01 can say why sanitizer interposition may matter, but only M004/R011 can prove whether a DMTCP wrapper, direct libc lookup, or link-map replacement avoids the interceptor without breaking symbol semantics. [src/dlwrappers.cpp] [src/dmtcp_dlsym.cpp] | Follow-up experiment required. |

## TSAN feasibility assessment

Feasibility answer for S01: consolidation is plausibly helpful for the motivating TSAN scenario because it can reduce DMTCP's own preload layering and make the DMTCP constructor/plugin-registration plan explicit, but it is not sufficient evidence that TSAN will run safely. The hypothesized failure path is not only that DMTCP has too many internal libraries; it is also that TSAN can run first, use `dlsym(RTLD_NEXT, ...)`, enter DMTCP's external `dlsym` wrapper, and then intercept DMTCP's direct `dl_iterate_phdr` call. [src/dlwrappers.cpp] [src/dmtcpworker.cpp]

The constructor side is encouraging but incomplete. DMTCP's constructor path is explicit and early once `libdmtcp.so` is loaded: priority 101, guard against repeat initialization, wrapper preparation, plugin-manager initialization, then event delivery. However, the motivating experiment says TSAN's constructor can run before `dmtcp_initialize_entry_point`; S01 has not executed that binary under a sanitizer and therefore cannot claim the external constructor ordering is fixed. [src/dmtcpworker.cpp]

The wrapper side remains the high-risk unknown. Internal wrapper preparation uses DMTCP's link-map based default-symbol search rather than calling the public `dlsym` symbol, but the external `dlsym` wrapper still handles `RTLD_NEXT` by calling `dl_iterate_phdr`. The reported TSAN interceptor stack hypothesis lines up with that source path. [src/syscallsreal.c] [src/dmtcp_dlsym_wrappers.cpp] [src/dlwrappers.cpp]
...
Why TSAN may intercept it: the motivating reproducer for future M004 work has TSAN's constructor entering first and `___interceptor_dl_iterate_phdr` above `dlsym_with_rtld_next` in the stack. Since this DMTCP path calls `dl_iterate_phdr` by symbol name from inside the wrapper, a sanitizer-provided interceptor earlier in the active search/interposition path can see that call unless DMTCP has a verified bypass. [src/dlwrappers.cpp]
...
Bypass option B is to add a DMTCP-controlled `dl_iterate_phdr` wrapper or real-function path that reaches libc/loader behavior before TSAN's interceptor. This experiment has a strict ordering requirement: the DMTCP wrapper or bypass must be first for the relevant call and still allow the rest of DMTCP to run late enough near libc for ordinary wrappers. [src/dlwrappers.cpp]
...
Consolidation cannot prove TSAN constructor safety. The motivating concern depends on the target being built with TSAN, TSAN's constructor order, and TSAN's interceptor behavior during DMTCP initialization; S01 only maps source paths and does not run that dynamic scenario. [src/dmtcpworker.cpp]
...
1. Exact TSAN constructor ordering under `bin/dmtcp_launch` is still empirical. Source shows DMTCP constructor priority 101, but TSAN's constructor and interceptor order come from the target's sanitizer runtime and must be observed with breakpoints. [src/dmtcpworker.cpp]
...
|---|---|---|---|---|
| Tiny `-fsanitize=thread` target under `bin/dmtcp_launch` | Reproduce the motivating sanitizer scenario on a minimal dynamically linked target and confirm whether the current branch reaches the sanitizer/DMTCP interaction. | The scenario defines the compile/launch shape and identifies TSAN as the motivating target; the source risk is DMTCP's external `dlsym(RTLD_NEXT)` path. [src/dlwrappers.cpp] | The target starts under `bin/dmtcp_launch`, reaches both TSAN and DMTCP breakpoints, and any crash stack is captured with the wrapper path named. | Startup crashes or loops before both breakpoints, or the stack cannot identify whether DMTCP's `dlsym` wrapper was involved. |
| Breakpoint/order confirmation for `__tsan::Initialize` and `dmtcp_initialize_entry_point` | Establish external constructor order rather than assuming source priority controls sanitizer order. | DMTCP constructor entry is `dmtcp_initialize_entry_point`; the planned debugger probe uses `__tsan::Initialize` as the sanitizer breakpoint. [src/dmtcpworker.cpp] | The debugger transcript records which breakpoint fires first and whether DMTCP initialization begins from a TSAN-triggered wrapper path. | Breakpoints are optimized out, never reached, or show an order that invalidates the hypothesized crash path. |
...
| `dl_iterate_phdr` interception or bypass prototype | Decide whether to wrap `dl_iterate_phdr`, bypass it, or replace it for the external `dlsym(RTLD_NEXT)` branch. | Risky call is in `dlsym_with_rtld_next`; candidate experiments include a DMTCP wrapper or forced libc call. [src/dlwrappers.cpp] | Prototype avoids `___interceptor_dl_iterate_phdr` during DMTCP initialization while preserving intended `dlsym(RTLD_NEXT)` results. | TSAN still intercepts the call, DMTCP loops/crashes, or symbol lookup semantics regress. |
...
7. Consolidation should reduce DMTCP's internal preload layers and make registration deterministic, but S02/S03 must not claim that this alone proves TSAN-safe ordering or removes the need for the M004/R011 `dl_iterate_phdr` experiments. [src/dlwrappers.cpp]

Narrow conclusion for S01: consolidation design must account for dynamic-linker search order, plugin-registration/event order, and sanitizer interposition. Moving internal plugin wrappers into one library reduces DMTCP's own preload layering, but it does not guarantee TSAN-safe wrapper ordering while the external `dlsym(RTLD_NEXT)` path can still reach `dl_iterate_phdr` and while constructor order with TSAN remains unproven at runtime. [src/dmtcp_launch.cpp] [src/pluginmanager.cpp] [src/dlwrappers.cpp]
